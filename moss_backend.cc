      ...
      if (!s->log()) {
        ...
        try {
          if (log_dir) {
            ...
            snprintf(temp_str, len, "%s%smoss_backend.log", log_dir, PATH_SEPARATOR);
          }
          m_log = new Logger("backend", log_dir ? temp_str : "moss_backend.log", Logger::str_to_level(log_level));
          s->set_logger(m_log);
...
int main(int argc, char *argv[]) {
  ...
  if (bp->bind_addr_name && bp->bind_addr_name[0] != '\0') {
    const char *result = resolve_hostname(bp->bind_addr_name, &bind_addr.sin_addr.s_addr);
    if (result) {
      log_err(log, "Could not resolve \"%s\": %s\n", bp->bind_addr_name, result);
      return_value = 1;
    }
  } else {
  ...
  try {
    if (bp->log_dir) {
      ...
      temp_str = new char[len];
      snprintf(temp_str, len, "%s%smoss_backend.log", bp->log_dir, PATH_SEPARATOR);
    }
    log = new Logger("backend", bp->log_dir ? temp_str : "moss_backend.log", Logger::str_to_level(bp->log_level));
    bp->set_logger(log);
  } catch (const std::bad_alloc&) {
  }
  if (temp_str) {
