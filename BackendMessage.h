...
class VaultNode;
// to not require everything to include VaultNode
...
public:
  ...
  virtual size_t message_len() const {
    return read32(m_header, 0);
  }
  /*
   * Additional accesors.
   */
  uint32_t get_id1() const {
    return read32(m_header, 8);
  }
  uint32_t get_id2() const {
...
public:
  typedef enum {
    UNKNOWN = 0, IN_DOUBT = 1, NO_STATE = 2, NEW_LOGIN = 3, // translates to ERROR_LOGGED_IN_ELSEWHERE in AuthServer
    AUTH_DISCONNECT = 4
  } kill_reason_t;
  ...
  kill_reason_t why() const {
    return (kill_reason_t) le32toh(m_reason);
  }
  kinum_t kinum() const {
    return le32toh(m_ki);
  }
...
public:
  ...
  authtype_t authtype() const {
    return (authtype_t) le32toh(m_authtype);
  }
  // KEEP little-endian as they are hashed in that order
  uint32_t server_nonce() const {
    return m_server;
  }
  uint32_t client_nonce() const {
    return m_client;
  }
...
public:
  // pre-send
  VaultPlayerCreate_FromBackendMessage(uint32_t id1, uint32_t id2, uint32_t reqid, status_code_t result, kinum_t kinum = 0,
...
public:
  ...
  virtual int16_t uru_msgtype() const {
    return le16toh(m_msgtype);
  }
...
class VaultNodeFetch_FromBackendMessage: public VaultPassthrough_BackendMessage {
public:
  // pre-send; note the _message_ is responsible for deleting the VaultNode
  VaultNodeFetch_FromBackendMessage(uint32_t id1, uint32_t id2, uint32_t reqid, status_code_t result, VaultNode *node);
...
public:
  ...
  uint32_t parent() const {
    return le32toh(m_parent);
  }
  uint32_t child() const {
    return le32toh(m_child);
  }
  // for add only
  uint32_t owner() const {
    return le32toh(m_owner);
  }
...
public:
  ...
  UruString* instance_name() const {
    return m_instance;
  }
  UruString* user_defined_name() const {
    return m_username;
  }
  UruString* display_name() const {
    return m_dispname;
  }
  uint32_t unk() const {
    return le32toh(m_unk);
  }
...
public:
  ...
  kinum_t player() const {
    return (kinum_t) le32toh(m_player);
  }
  uint32_t nodeid() const {
    return le32toh(m_nodeid);
  }
...
public:
  ...
  kinum_t holder() const {
    return (kinum_t) le32toh(m_holder);
  }
  UruString* score_name() const {
    return m_name;
  }
...
class VaultScoreGet_FromBackendMessage: public VaultPassthrough_BackendMessage {
public:
  // pre-send, has error
  VaultScoreGet_FromBackendMessage(uint32_t id1, uint32_t id2, uint32_t reqid, status_code_t result);
  // pre-send, no error
  VaultScoreGet_FromBackendMessage(uint32_t id1, uint32_t id2, uint32_t reqid, status_code_t result, uint32_t score_id,
      kinum_t holder, int32_t create_time, uint32_t type, int32_t score_value, UruString *score_name);
...
public:
  ...
  uint32_t score_id() const {
    return le32toh(m_scoreid);
  }
  int32_t delta() const {
    return le32toh(m_delta);
  }
...
public:
  typedef enum {
    ST_NONE = 0, ST_HOSTNAME = 1, ST_IPADDR = 2
  } address_type_t;

  // pre-send
  TrackServiceTypes_BackendMessage(uint32_t id1, uint32_t id2, bool auth_enabled, bool file_enabled, bool game_enabled,
      uint32_t ip_address);
  TrackServiceTypes_BackendMessage(uint32_t id1, uint32_t id2, bool auth_enabled, bool file_enabled, bool game_enabled,
  ...
  address_type_t addrtype() const {
    return (address_type_t) m_addrtype;
  }
  ...
  uint32_t request_pushes() const {
    return le32toh(m_request_pushes);
  }
...
public:
  ...
  uint32_t reqid2() const {
    return le32toh(m_reqid2);
  } // for future expansion
  bool wants_file() const {
    return (m_want_file != 0);
  }
...
public:
  typedef enum {
    INVALID = 0, GLOBAL_INIT = 1, GLOBAL_UPDATE = 2, // not currently used/supported
    VAULT_SDL_UPDATE = 3,
    VAULT_SDL_LOAD = 4
  } sdl_type_t;
  ...
  // accessors
  kinum_t sent_by() const {
    return (kinum_t) le32toh(m_kinum);
  }
  uint32_t from_id1() const {
    return le32toh(m_from_id1);
  }
  ...
  uint32_t sdl_len() const {
    return le32toh(m_datalen);
  }
  sdl_type_t update_type() const {
    return (sdl_type_t) le32toh(m_sdl_type);
  }
...
public:
  ...
  uint32_t how_many() const {
    return le32toh(m_number);
  }
  uint32_t start_at() const {
    return le32toh(m_start);
  }
...
public:
  ...
  problem_t problem() const {
    return (problem_t) le32toh(m_problem);
  }
...
public:
  ...
  virtual ~TrackAddPlayer_FromBackendMessage() {
    if (m_name)
      delete m_name;
  }
...
public:
  ...
  uint32_t requester() const {
    return le32toh(m_requester);
  }
  uint32_t localid() const {
    return le32toh(m_localid);
  }
  // for message re-use
...
public:
  ...
  int32_t number() const {
    return le32toh(m_data->number);
  }
  UruString* name() const {
    return m_name;
  }
  UruString* agename() const {
    return m_agename;
  }
...
public:
  // pre-send
  MarkersAll_BackendMessage(uint32_t id1, uint32_t id2, uint32_t localid, uint32_t gameid, size_t num_entries);
  // note: these "double"s are all little-endian order, never host order
  void add_marker(int32_t number, double x, double y, double z, UruString &name, UruString &age);
  void finalize();
  ...
  const marker_data_t* data() const {
    return m_current;
  }
  UruString* name() const {
    return m_name;
...
public:
  // pre-send
  MarkersCaptured_BackendMessage(uint32_t id1, uint32_t id2, uint32_t localid, uint32_t gameid, size_t num_entries);
...
public:
  // pre-send
  MarkerGameRename_BackendMessage(uint32_t id1, uint32_t id2, bool server, uint32_t gameid, uint32_t localid,
...
public:
  // pre-send
  MarkerGameRenameMarker_BackendMessage(uint32_t id1, uint32_t id2, bool server, uint32_t gameid, uint32_t localid,
...
public:
  // pre-send
  MarkerGameDeleteMarker_BackendMessage(uint32_t id1, uint32_t id2, bool server, uint32_t gameid, uint32_t localid,
      int32_t number);
...
public:
  // pre-send
  MarkerGameCaptureMarker_BackendMessage(uint32_t id1, uint32_t id2, bool server, uint32_t gameid, uint32_t localid,
