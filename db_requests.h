...
public:
  ...
  AuthAcctLogin_AcctQuery_Result(AuthAcctLogin_AcctQuery_Result &other) :
      result_code(other.result_code), user_class(other.user_class), is_visitor(other.is_visitor) {
...
public:
  AuthAcctLogin_AcctQuery(const char *name, AuthAcctLogin_AcctQuery_Result &result) :
      pqxx::transactor<>("AuthAcctLogin_AcctQuery"), m_name(name), m_result(result) {
  }

  AuthAcctLogin_AcctQuery(const AuthAcctLogin_AcctQuery &other) :
      pqxx::transactor<>("AuthAcctLogin_AcctQuery"), m_name(other.m_name), m_result(other.m_result) {
  }
  ...
  void operator()(argument_type &T) {
    ...
    if (R.size() == 1 && R.columns() == 5) {
      ...
      F = R[0]["id"];
      if (F.is_null() || uuid_string_to_bytes(m_result.uuid, UUID_RAW_LEN, F.c_str(), strlen(F.c_str()), 1, 1)) {
...
public:
  ...
  AuthAcctLogin_PlayerQuery_Player(const AuthAcctLogin_PlayerQuery_Player &other) :
      kinum(other.kinum), name(other.name), gender(other.gender), explorer_type(other.explorer_type) {
  }
...
public:
  ...
  AuthValidateKI(const AuthValidateKI &other) :
      pqxx::transactor<pqxx::nontransaction>("AuthValidateKI"), m_uuid(other.m_uuid), m_kinum(other.m_kinum), m_result(
          other.m_result), m_name(other.m_name) {
  }
...
public:
  ...
  AuthChangePassword(const AuthChangePassword &other) :
      pqxx::transactor<>("AuthChangePassword"), m_uuid(other.m_uuid), m_name(other.m_name), m_hash(other.m_hash), m_result(
...
public:
  ...
  AuthVerifyPassword(const AuthVerifyPassword &other) :
      pqxx::transactor<pqxx::nontransaction>("AuthVerifyPassword"), m_uuid(other.m_uuid), m_name(other.m_name), m_hash(
          other.m_hash), m_result(other.m_result) {
  }
...
public:
  ...
  VaultPlayerRequest_Verify(const VaultPlayerRequest_Verify &other) :
      pqxx::transactor<pqxx::nontransaction>("VaultPlayerRequest_Verify"), m_uuid(other.m_uuid), m_name(other.m_name), m_result(
          other.m_result) {
  }
...
public:
  VaultPlayerDelete_Request(kinum_t kinum, status_code_t &result, std::multimap<kinum_t, uint32_t> &tell_who,
      uint32_t &player_info) :
      pqxx::transactor<>("VaultPlayerDelete_Request"), m_kinum(kinum), m_result(result), m_tell_who(tell_who), m_player_info(
          player_info), my_result(ERROR_INTERNAL) {
  }

  VaultPlayerDelete_Request(const VaultPlayerDelete_Request &other) :
      pqxx::transactor<>("VaultPlayerDelete_Request"), m_kinum(other.m_kinum), m_result(other.m_result), m_tell_who(
          other.m_tell_who), m_player_info(other.m_player_info), my_result(other.my_result) {
  }
...
public:
  ...
  VaultFetchRefs_Request(const VaultFetchRefs_Request &other) :
      pqxx::transactor<pqxx::nontransaction>("VaultFetchRefs_Request"), m_node(other.m_node), m_result(other.m_result), m_list(
          other.m_list) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM fetchnoderefs(" << m_node << ")";
    pqxx::result R(T.exec(qstr));
...
public:
  VaultFindNode_Request(uint32_t node, status_code_t &result, uint32_t &val) :
      pqxx::transactor<pqxx::nontransaction>("VaultFindNode_Request"), m_owner(node), m_result(result), m_val(val) {
  }

  VaultFindNode_Request(const VaultFindNode_Request &other) :
      pqxx::transactor<pqxx::nontransaction>("VaultFindNode_Request"), m_owner(other.m_owner), m_result(other.m_result), m_val(
          other.m_val) {
  }
  ...
  void operator()(argument_type &T) {
    ...
    qstr << "SELECT nodeid FROM playerinfo INNER JOIN noderefs ON "
        << "playerinfo.nodeid = noderefs.child where noderefs.parent = " << m_owner;
    pqxx::result R(T.exec(qstr));
...
public:
  ...
  VaultFetchNode_Request(const VaultFetchNode_Request &other) :
      pqxx::transactor<pqxx::nontransaction>("VaultFetchNode_Request"), m_id(other.m_id), m_result(other.m_result), m_node(
          other.m_node), m_log(other.m_log) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM fetchnode(" << m_id << ")";
    pqxx::result R(T.exec(qstr));
    ...
    for (u_int i = 1; i < 32; i++) {
      ...
      if (bits & bit) {
        ...
        if (F.is_null()) {
          ...
          case VaultNode::STRING:
            {
              UruString str((const u_char*) F.c_str(), strlen(F.c_str()) + 1, false, false, false/* unneeded*/);
...
public:
  VaultSaveNode_Request(uint32_t nodeid, const VaultNode *node, status_code_t &result, Logger *log) :
      pqxx::transactor<>("VaultSaveNode_Request"), m_id(nodeid), m_node(node), m_result(result), m_log(log) {
  }

  VaultSaveNode_Request(const VaultSaveNode_Request &other) :
      pqxx::transactor<>("VaultSaveNode_Request"), m_id(other.m_id), m_node(other.m_node), m_result(other.m_result), m_log(
          other.m_log) {
  }
...
public:
  ...
  VaultCreateNode_Request(const VaultCreateNode_Request &other) :
      pqxx::transactor<>("VaultCreateNode_Request"), m_node(other.m_node), m_creatoracctid(other.m_creatoracctid), m_creatorid(
          other.m_creatorid), m_id(other.m_id), my_id(other.my_id), m_log(other.m_log) {
  }
  ...
  void operator()(argument_type &T) {
    ...
    std::stringstream qstr;
    qstr << "SELECT * FROM newnodeid(" << (uint32_t) ntype << ")";
    pqxx::result R(T.exec(qstr));
    if (R.size() != 1) {
    ...
    vals << " (" << my_id << ", now(), now(), '" << ESC_STR(T, acct_str) << "', " << m_creatorid;
    uint32_t bits = VaultNode::all_bits_for_type(ntype);
    // the DB at the moment has separate tables for each node type, so
    // we don't put the node type into the query 
    bits &= ~NodeType;
    ...
    qstr2 << "INSERT INTO " << VaultNode::tablename_for_type(ntype) << cols.str() << ") VALUES " << vals.str() << ")";
...
public:
  VaultAddRef_Request(uint32_t parent, uint32_t child, uint32_t owner, status_code_t &result) :
      pqxx::transactor<>("VaultAddRef_Request"), m_parent(parent), m_child(child), m_owner(owner), m_result(result), my_result(
          result) {
  }

  VaultAddRef_Request(const VaultAddRef_Request &other) :
      pqxx::transactor<>("VaultAddRef_Request"), m_parent(other.m_parent), m_child(other.m_child), m_owner(other.m_owner), m_result(
          other.m_result), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM addnode(" << m_parent << ", " << m_child << ", " << m_owner << ")";
...
public:
  VaultAgeList_Request(UruString &filename, status_code_t &result, std::vector<VaultAgeList_AgeInfo> &ages) :
      pqxx::transactor<pqxx::nontransaction>("VaultAgeList_Request"), m_filename(filename), m_list(ages), m_result(result) {
  }

  VaultAgeList_Request(const VaultAgeList_Request &other) :
      pqxx::transactor<pqxx::nontransaction>("VaultAgeList_Request"), m_filename(other.m_filename), m_list(other.m_list), m_result(
          other.m_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM getpublicagelist('" << ESC_STR(T, m_filename.c_str()) << "')";
    pqxx::result R(T.exec(qstr));
...
public:
  VaultSendNode_Request(kinum_t player, uint32_t nodeid, kinum_t sender, status_code_t &result, uint32_t &inboxid) :
      pqxx::transactor<>("VaultSendNode_Request"), m_player(player), m_node(nodeid), m_sender(sender), m_result(result), m_inbox(
          inboxid), my_result(result), my_inbox(0) {
  }
  ...
  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM sendnode(" << m_player << ", " << m_node << ", " << m_sender << ")";
...
public:
  ...
  void operator()(argument_type &T) {
    ...
    qstr << "UPDATE ageinfo SET int32_2=" << (m_public ? "1" : "0") << ", modifytime=now() WHERE nodeid=" << m_node;
    T.exec(qstr);
    my_result = NO_ERROR;
  }
...
public:
  ...
  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM getscore(" << m_score_holder << ", '" << ESC_STR(T, m_score_name->c_str()) << "')";
    pqxx::result R(T.exec(qstr));
...
public:
  ...
  VaultAddToScore_Request(const VaultAddToScore_Request &other) :
      pqxx::transactor<>("VaultAddToScore_Request"), m_score_id(other.m_score_id), m_delta(other.m_delta), m_result(
          other.m_result), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM addtoscore(" << m_score_id << ", " << m_delta << ")";
    pqxx::result R(T.exec(qstr));
    if (R.size() != 1) {
      my_result = ERROR_INTERNAL;
    } else {
...
public:
  VaultTransferScore_Request(uint32_t score_id, uint32_t dest_id, int32_t delta, status_code_t &result) :
      pqxx::transactor<>("VaultTransferScore_Request"), m_score_id(score_id), m_dest_id(dest_id), m_delta(delta), m_result(
          result), my_result(result) {
  }

  VaultTransferScore_Request(const VaultTransferScore_Request &other) :
      pqxx::transactor<>("VaultTransferScore_Request"), m_score_id(other.m_score_id), m_dest_id(other.m_dest_id), m_delta(
          other.m_delta), m_result(other.m_result), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM transferscore(" << m_score_id << ", " << m_dest_id << ", " << m_delta << ")";
    pqxx::result R(T.exec(qstr));
    if (R.size() != 1) {
      my_result = ERROR_INTERNAL;
    } else {
...
public:
  ...
  MarkerGameCreate_Request(const MarkerGameCreate_Request &other) :
      pqxx::transactor<>("MarkerGameCreate_Request"), m_owner(other.m_owner), m_game_name(other.m_game_name), m_type(
          other.m_type), m_game_id(other.m_game_id), m_uuid(other.m_uuid), m_result(other.m_result), my_game_id(
          other.my_game_id), my_result(other.my_result) {
...
public:
  MarkerGameRename_Request(uint32_t internal_id, UruString *game_name, status_code_t &result) :
      pqxx::transactor<>("MarkerGameRename_Request"), m_game_id(internal_id), m_game_name(game_name), m_result(result), my_result(
          result) {
  }

  MarkerGameRename_Request(const MarkerGameRename_Request &other) :
      pqxx::transactor<>("MarkerGameRename_Request"), m_game_id(other.m_game_id), m_game_name(other.m_game_name), m_result(
          other.m_result), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM renamemarkergame(" << m_game_id << ", '" << ESC_STR(T, m_game_name->c_str()) << "')";
    pqxx::result R(T.exec(qstr));
    if (R.size() != 1) {
      my_result = ERROR_INTERNAL;
    } else {
...
public:
  MarkerGameDelete_Request(uint32_t internal_id, status_code_t &result) :
      pqxx::transactor<>("MarkerGameDelete_Request"), m_game_id(internal_id), m_result(result), my_result(result) {
  }

  MarkerGameDelete_Request(const MarkerGameDelete_Request &other) :
      pqxx::transactor<>("MarkerGameDelete_Request"), m_game_id(other.m_game_id), m_result(other.m_result), my_result(
          other.my_result) {
  }
...
public:
  ...
  MarkerGameAddMarker_Request(const MarkerGameAddMarker_Request &other) :
      pqxx::transactor<>("MarkerGameAddMarker_Request"), m_game_id(other.m_game_id), m_x(other.m_x), m_y(other.m_y), m_z(
          other.m_z), m_name(other.m_name), m_age(other.m_age), m_marker(other.m_marker), m_result(other.m_result), my_marker(
          other.my_marker), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM addmarker(" << m_game_id << ", " << m_x << ", " << m_y << ", " << m_z << ", '"
        << ESC_STR(T, m_name->c_str()) << "', '" << ESC_STR(T, m_age->c_str()) << "')";
    pqxx::result R(T.exec(qstr));
    if (R.size() != 1) {
      my_result = ERROR_INTERNAL;
    } else {
...
public:
  ...
  MarkerGameRenameMarker_Request(const MarkerGameRenameMarker_Request &other) :
      pqxx::transactor<>("MarkerGameRenameMarker_Request"), m_game_id(other.m_game_id), m_marker(other.m_marker), m_name(
          other.m_name), m_result(other.m_result), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM renamemarker(" << m_game_id << ", " << m_marker << ", '" << ESC_STR(T, m_name->c_str()) << "')";
    pqxx::result R(T.exec(qstr));
    if (R.size() != 1) {
      my_result = ERROR_INTERNAL;
    } else {
...
public:
  ...
  MarkerGameDeleteMarker_Request(const MarkerGameDeleteMarker_Request &other) :
      pqxx::transactor<>("MarkerGameDeleteMarker_Request"), m_game_id(other.m_game_id), m_marker(other.m_marker), m_result(
          other.m_result), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM deletemarker(" << m_game_id << ", " << m_marker << ")";
    pqxx::result R(T.exec(qstr));
    if (R.size() != 1) {
      my_result = ERROR_INTERNAL;
    } else {
...
#ifdef USE_PQXX
class MarkerGameMarkers_Request: public pqxx::transactor<pqxx::nontransaction> {
public:
  MarkerGameMarkers_Request(uint32_t internal_id, status_code_t &result, std::vector<MarkerGame_MarkerInfo> &markerlist) :
      pqxx::transactor<pqxx::nontransaction>("MarkerGameMarkers_Request"), m_game_id(internal_id), m_list(markerlist), m_result(
          result) {
  }
...
class MarkerGameCaptured_Request: public pqxx::transactor<pqxx::nontransaction> {
public:
  MarkerGameCaptured_Request(uint32_t internal_id, kinum_t player, status_code_t &result,
      std::vector<MarkerGame_CapturedMarker> &markers) :
      pqxx::transactor<pqxx::nontransaction>("MarkerGameCaptured_Request"), m_game_id(internal_id), m_player(player), m_list(
          markers), m_result(result) {
  }

  MarkerGameCaptured_Request(const MarkerGameCaptured_Request &other) :
      pqxx::transactor<pqxx::nontransaction>("MarkerGameCaptured_Request"), m_game_id(other.m_game_id), m_player(
          other.m_player), m_list(other.m_list), m_result(other.m_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM capturedmarkers(" << m_game_id << ", " << m_player << ")";
    pqxx::result R(T.exec(qstr));
...
public:
  ...
  MarkerGameCaptureMarker_Request(const MarkerGameCaptureMarker_Request &other) :
      pqxx::transactor<>("MarkerGameCaptureMarker_Request"), m_game_id(other.m_game_id), m_player(other.m_player), m_marker(
          other.m_marker), m_value(other.m_value), m_result(other.m_result), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM setmarkerto(" << m_game_id << ", " << m_player << ", " << m_marker << ", " << m_value << ")";
    pqxx::result R(T.exec(qstr));
    if (R.size() != 1) {
      my_result = ERROR_INTERNAL;
    } else {
...
public:
  MarkerGameStop_Request(uint32_t internal_id, kinum_t player, status_code_t &result) :
      pqxx::transactor<>("MarkerGameStop_Request"), m_game_id(internal_id), m_player(player), m_result(result), my_result(
          result) {
  }

  MarkerGameStop_Request(const MarkerGameStop_Request &other) :
      pqxx::transactor<>("MarkerGameStop_Request"), m_game_id(other.m_game_id), m_player(other.m_player), m_result(
          other.m_result), my_result(other.my_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT stopmarkergame(" << m_game_id << ", " << m_player << ")";
    pqxx::result R(T.exec(qstr));
    my_result = NO_ERROR;
  }
...
public:
  ...
  VaultGetAgeByUUID(const VaultGetAgeByUUID &other) :
      pqxx::transactor<pqxx::nontransaction>("VaultGetAgeByUUID"), m_uuid(other.m_uuid), m_age_node(other.m_age_node), m_age_info(
          other.m_age_info), m_age_name(other.m_age_name), m_result(other.m_result) {
  }
...
public:
  SetPlayerOffline(kinum_t kinum, bool &was_online, uint32_t &info_node, status_code_t &result) :
      pqxx::transactor<>("SetPlayerOffline"), m_kinum(kinum), m_online(was_online), m_node(info_node), m_result(result), my_online(
          true), my_node(0), my_result(ERROR_INTERNAL) {
  }

  SetPlayerOffline(const SetPlayerOffline &other) :
      pqxx::transactor<>("SetPlayerOffline"), m_kinum(other.m_kinum), m_online(other.m_online), m_node(other.m_node), m_result(
          other.m_result), my_online(other.my_online), my_node(other.my_node), my_result(other.my_result) {
  }
...
public:
  SetPlayerConnected(kinum_t kinum, status_code_t &result) :
      pqxx::transactor<>("SetPlayerConnected"), m_kinum(kinum), m_result(result), my_result(ERROR_INTERNAL) {
  }
...
public:
  PlayersReferringTo(uint32_t node, status_code_t &result, std::vector<kinum_t> &players) :
      pqxx::transactor<pqxx::nontransaction>("PlayersReferringTo"), m_node(node), m_result(result), m_list(players), my_result(
          ERROR_INTERNAL) {
  }

  PlayersReferringTo(const PlayersReferringTo &other) :
      pqxx::transactor<pqxx::nontransaction>("PlayersReferringTo"), m_node(other.m_node), m_result(other.m_result), m_list(
          other.m_list), my_result(other.my_result) {
  }
...
public:
  ...
  AgeReferringTo(const AgeReferringTo &other) :
      pqxx::transactor<pqxx::nontransaction>("AgeReferringTo"), m_node(other.m_node), m_uuid(other.m_uuid), m_result(
          other.m_result), my_result(other.my_result) {
    memcpy(my_uuid, other.my_uuid, UUID_RAW_LEN);
  }
...
public:
  DeleteAge(uint32_t ageinfo_node, status_code_t &result) :
      pqxx::transactor<>("DeleteAge"), m_age(ageinfo_node), m_result(result), my_result(ERROR_INTERNAL) {
  }

  DeleteAge(const DeleteAge &other) :
      pqxx::transactor<>("DeleteAge"), m_age(other.m_age), m_result(other.m_result), my_result(other.my_result) {
  }
...
public:
  ...
  GetGlobalSDL(const GetGlobalSDL &other) :
      pqxx::transactor<pqxx::nontransaction>("GetGlobalSDL"), m_filename(other.m_filename), m_outbuf(other.m_outbuf), m_buflen(
          other.m_buflen), m_result(other.m_result) {
  }

  void operator()(argument_type &T) {
    pqxx::result R(T.exec("SELECT * FROM getglobalsdlbyname('" + ESC_STR(T, m_filename.c_str()) + "')"));
    if (*m_outbuf) {
      // this should not happen
      throw std::runtime_error("We appear to have restarted what should be a "
          "nontransaction which only sets local state after "
...
public:
  ...
  GetVaultSDL(const GetVaultSDL &other) :
      pqxx::transactor<pqxx::nontransaction>("GetVaultSDL"), m_age(other.m_age), m_filename(other.m_filename), m_outbuf(
          other.m_outbuf), m_buflen(other.m_buflen), m_result(other.m_result) {
  }

  void operator()(argument_type &T) {
    std::stringstream qstr;
    qstr << "SELECT * FROM getagesdl(" << m_age << ", '" << ESC_STR(T, m_filename.c_str()) + "')";
    pqxx::result R(T.exec(qstr));
...
public:
  ...
  GetAgeUUIDFor(const GetAgeUUIDFor &other) :
      pqxx::transactor<pqxx::nontransaction>("GetAgeUUIDFor"), m_node(other.m_node), m_uuid(other.m_uuid), m_result(
          other.m_result) {
  }
  ...
  void operator()(argument_type &T) {
    ...
    if (F.is_null()) {
      // empty result
      m_result = ERROR_NODE_NOT_FOUND;
    } else if (uuid_string_to_bytes(m_uuid, UUID_RAW_LEN, F.c_str(), strlen(F.c_str()), 1, 1)) {
      m_result = ERROR_INVALID_PARAM;
    } else {
      m_result = NO_ERROR;
    }
...
public:
  ...
  Egg1(const Egg1 &other) :
      pqxx::transactor<>("Egg1"), m_kinum(other.m_kinum), m_parent(other.m_parent), m_child(other.m_child), m_result(
          other.m_result), my_parent(other.my_parent), my_child(other.my_child), my_result(other.my_result) {
  }
