...
public:
  ...
  typedef enum {
    NO_SHUTDOWN = 0, FORGET_THIS_CONNECTION, // NOT fatal, used when connections change threads
    CLIENT_CLOSE,
    CLIENT_TIMEOUT,
    SERVER_SHUTDOWN,
    PEER_SHUTDOWN,
  ...
  virtual void conn_completed(Connection *conn) {
  ...
  // only.
  virtual reason_t conn_timeout(Connection *conn, reason_t why) {
    return why;
  }
  ...
  // conn_timeout().
  virtual reason_t conn_shutdown(Connection *conn, reason_t why) {
    return why;
  }
  ...
  virtual bool shutdown(reason_t reason) {
    return true;
  }
  ...
  Server(const char *server_dir, bool is_thread) :
      m_serv_dir(server_dir), m_is_thread(is_thread), m_signal_flags(NULL), m_signal_ct(0), m_signal_processor(NULL), m_log(
          NULL), m_is_child(true), m_fd(-1), m_ipaddr(0), m_id(0), m_shutdown_done(false), m_in_shutdown(false) {
  ...
  std::list<Connection*>& get_conn_list() {
    return m_conns;
  }
...
public:
  ...
  public:
    ...
    void set_in_connect(bool c) {
      m_in_connect = c;
    }
    bool in_shutdown() const {
      return m_in_shutdown;
    }
    void set_in_shutdown(bool s) {
      m_in_shutdown = s;
    }
    void enqueue(NetworkMessage *msg, MessageQueue::priority_t p = MessageQueue::NORMAL) {
      m_msg_queue->enqueue(msg, p);
    }
    size_t queue_size() const {
      return m_msg_queue->size();
    }
    MessageQueue* msg_queue() const {
      return m_msg_queue;
    }
  ...
  public:
    ...
    std::deque<Timer*>::const_iterator begin() {
      return m_queue.begin();
    }
    std::deque<Timer*>::const_iterator end() {
      return m_queue.end();
    }
  protected:
