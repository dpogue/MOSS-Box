...
UruString::UruString(const char *c_str, bool copy) :
    m_cstr(NULL), m_altstr(NULL), m_alt_type(C), m_owns_cstr(false), m_owns_altstr(false), m_arrival_len(0), m_cstr_size(0), m_altstr_size(
        0) {
  if (!c_str) {
    return;
  }
...
UruString::UruString(const std::string &str, bool copy) :
    m_cstr(NULL), m_altstr(NULL), m_alt_type(C), m_owns_cstr(false), m_owns_altstr(false), m_arrival_len(0) {
  ...
  if (m_altstr && ((m_alt_type ^ desired_type) == BITFLIP)) {
    ...
    if (as_wide && !(source_type & WIDE)) {
      ...
      if (iconv_state != (iconv_t) -1) {
        ...
        // iconv is broken or something
        read_at.u = (tempbufr ? tempbufr : ((source_type & HAS_LEN) ? source_str.u + 2 : source_str.u));
...
size_t UruString::send_len(bool include_length, bool as_wide, bool include_null) {
  size_t result;
  if (as_wide) {
    if (!m_altstr || !(m_alt_type & WIDE)) {
