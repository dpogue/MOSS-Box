 ...
 */
static bool msg_not_handled(PropagateBufferMessage *msg) {
  return false;
}
static bool msg_is_handled(PropagateBufferMessage *msg) {
  return true;
}
static bool check_useable_none(PropagateBufferMessage *msg) {
  return true;
}
...
static bool bad_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log) {
  log_err(log, "handle_message should never be called for subtype 0x%04x\n", msg->subtype());
  return false;
}
static bool ignore_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log) {
  const char *msgtype;
  switch (msg->subtype()) {
...
static bool passthrough_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log) {
 ...
 */
static bool members_list_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log) {
...
static bool state_request_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn,
    Logger *log) {
...
static propagate_handler ph_state_request = { msg_is_handled, state_request_check_useable, state_request_handler };
...
static bool test_and_set_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log) {
  ...
  if (buflen < offset + claimed_msglen) {
    log_warn(log, "plNetMsgTestAndSet message shorter (%u) than claimed (%u) "
        "(kinum=%u) lock '%s'\n", buflen - offset, claimed_msglen, ki, lockname);
    goto test_and_set_truncated;
  }
  ...
  }
  {
    ...
    if (str != "TrigState") {
      ...
      if (log) {
        log->dump_contents(Logger::LOG_NET, buf + submsg_offset, buflen - submsg_offset);
      }
...
static propagate_handler ph_test_and_set = { msg_is_handled, header_only_check_useable, test_and_set_handler };
...
static propagate_handler ph_sdl = { msg_is_handled, header_only_check_useable, sdl_handler };
...
static bool directed_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log) {
...
static bool voice_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log) {
...
static bool load_clone_handler(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log) {
  ...
  const SDLDesc *clone_desc = SDLDesc::find_by_name("CloneMessage", state->sdl_descs());
  if (!clone_desc) {
...
static propagate_handler ph_load_clone = { msg_is_handled, load_clone_check_useable, load_clone_handler };
#endif /* !STANDALONE */
...
public:
  VarSyncGameMgr(uint32_t id) :
      GameMgr(id, VarSync) {
  }
  virtual ~VarSyncGameMgr() {
  }

  bool initialize_game(const GameMgrMessage *msg, kinum_t player, GameServer *server);
...
public:
  MarkerGameMgr(uint32_t id, uint32_t server_id1, uint32_t server_id2) :
      GameMgr(id, Marker), m_sid1(server_id1), m_sid2(server_id2), m_game_name(NULL), m_template(NULL), m_private_id(0), m_state(
          INIT), m_elapsed_time(0) {
  }
...
protected:
  ...
  public:
    ...
    DoorTimer(struct timeval &when, BlueSpiralGameMgr *mgr, GameServer *server, action_t why) :
...
bool VarSyncGameMgr::got_message(GameMgrMessage *msg, kinum_t player, GameServer *server) {
...
bool MarkerGameMgr::initialize_game(const GameMgrMessage *msg, kinum_t player, GameServer *server) {
  ...
  if (m_template->strlen() == 0) {
    ...
    if (uuid_string_to_bytes(uuid, UUID_RAW_LEN, m_template->c_str(), m_template->strlen(), true, true)) {
      ...
      MarkerGetGame_BackendMessage *get_game = new MarkerGetGame_BackendMessage(m_sid1, m_sid2, false, m_gameid, true,
          (uint32_t) player, m_game_type, uuid, m_game_name);
      server->send_to_vault(get_game);
      if (get_game->del_ref() < 1) {
        // shouldn't happen
        delete get_game;
...
bool MarkerGameMgr::got_message(GameMgrMessage *msg, kinum_t player, GameServer *server) {
  Logger *log = server->log();
  if (player != m_owner) {
...
bool MarkerGameMgr::process_backend_message(NetworkMessage *in, GameServer *server) {
  ...
  case MARKER_DUMP | FROM_SERVER:
    if (m_state != LIST_WAIT) {
      ...
      GameMgr_Marker_GameNameChanged_Message *name = new GameMgr_Marker_GameNameChanged_Message(m_gameid, m_game_name);
      server->send_to_ki(m_owner, name);
      if (name->del_ref() < 1) {
        // shouldn't happen
        delete name;
  ...
  case MARKER_STATE | FROM_SERVER:
    if (m_state < READY) {
      ...
      MarkersCaptured_BackendMessage *msg = (MarkersCaptured_BackendMessage*) in;
      size_t num_markers = msg->size();
      if (num_markers > 0) {
        // send messages to client
  ...
  case MARKER_GAME_RENAME | FROM_SERVER:
    if (m_state == INIT || m_state == START || m_state == DEAD) {
      ...
      GameMgr_Marker_GameNameChanged_Message *reply = new GameMgr_Marker_GameNameChanged_Message(m_gameid, msg->name());
      server->send_to_ki(m_owner, reply);
      if (reply->del_ref() < 1) {
        // shouldn't happen
        delete reply;
  ...
  case MARKER_GAME_DELETE | FROM_SERVER:
    if (m_state == INIT || m_state == START || m_state == DEAD) {
      ...
      if (success) {
        ...
        log_warn(log, "MarkerMgr %u: deletion of game (internal ID %u) failed\n", m_gameid, m_private_id);
      }
    }
    break;
...
bool BlueSpiralGameMgr::initialize_game(const GameMgrMessage *msg, kinum_t player, GameServer *server) {
  if (m_owner == 0) {
    m_owner = player;
  }
  //const uint8_t *buf = msg->buffer();
...
void BlueSpiralGameMgr::start_game(GameServer *server) {
  ...
  GameMgr_BlueSpiral_ClothOrder_Message *order = new GameMgr_BlueSpiral_ClothOrder_Message(m_gameid, m_order);
...
bool HeekGameMgr::initialize_game(const GameMgrMessage *msg, kinum_t player, GameServer *server) {
  if (m_owner == 0) {
    m_owner = player;
  }
  ...
  case Cli2Srv_Heek_PlayGame:
    if (msg->message_len() < off + 5 + 512) {
      ...
      log_debug(log, "Player kinum=%u game=%u current_game=%u\n",
        ...
        GameMgr_Heek_PlayGame_Message *playgame = new GameMgr_Heek_PlayGame_Message(m_gameid, true/*XXXever false?*/,
            (m_sitting_ct == 1) /*single*/, (m_sitting_ct > 1 && m_state < COUNTDOWN) /*enable*/);
    ...
    if (game_winners == 1 && m_pending_winner_ct == 0) {
      ...
      for (i = 0; i < 5; i++) {
        if (m_sitting[i] && m_sitting[i]->m_pending_winner) {
          ...
          totals[i] = m_sitting[i]->m_wins[0] + m_sitting[i]->m_wins[1] + m_sitting[i]->m_wins[2];
          max = MAX(max, totals[i]);
        }
      }
          ...
          if (totals[i] != max) {
            ...
            if (m_pending_winner_ct > 0) {
              if (m_pending_winner_ct < 2) {
                // yikes!
                log_err(server->log(), "Not enough pending winners in Heek sudden death!\n");
              } else {
    ...
    if (m_pending_winner_ct == 0 && game_winners > 1) {
      ...
      for (i = 0; i < 5; i++) {
        if (m_sitting[i] && m_sitting[i]->m_pending_winner) {
          ...
          light++;
          lightmsg = new GameMgr_Heek_Lights_Message(m_gameid, light, GameMgr_Heek_Lights_Message::Flash);
          server->send_to_ki(m_sitting[i]->m_ki, lightmsg);
          if (lightmsg->del_ref() < 1) {
            delete lightmsg; // wasn't queued
          }
...
void GameMgr::send_setup_reply(const GameMgrMessage *msg, kinum_t player, GameServer *server) {
  GameMgr_Setup_Reply *rmsg = new GameMgr_Setup_Reply(m_gameid, msg->reqid(), m_owner, msg->setup_uuid());
