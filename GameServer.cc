...
GameServer::GameServer(const char *server_dir, bool is_a_thread, struct sockaddr_in &vault_address, const uint8_t *uuid,
  ...
  m_client_queue = new std::deque<std::pair<GameConnection*, NetworkMessage*> >();
...
GameServer::~GameServer() {
  ...
  for (state = m_game_state.m_sdl.begin(); state != m_game_state.m_sdl.end(); state++) {
    delete *state;
  }
...
bool GameServer::shutdown(reason_t reason) {
  ...
  TrackGameBye_ToBackendMessage *bye = new TrackGameBye_ToBackendMessage(m_ipaddr, m_id, true);
...
Server::reason_t GameServer::message_read(Connection *conn, NetworkMessage *in) {
  if (conn == m_vault) {
  ...
  if (in->type() == -1) {
    ...
    if (!handler->msg_handled(prop)) {
      log_net(m_log, "Dropping unhandled game message type 0x%04x (kinum=%u)\n", prop->subtype(), player);
      if (m_log) {
        m_log->dump_contents(Logger::LOG_NET, prop->buffer(), prop->message_len());
      }
    } else if (!handler->check_useable(prop)) {
      // protocol error
      log_warn(m_log, "Game message too short! (kinum=%u)\n", player);
      if (m_log) {
        m_log->dump_contents(Logger::LOG_WARN, prop->buffer(), prop->message_len());
      }
      delete in;
      return PROTOCOL_ERROR;
    } else {
    ...
    if (!mgr->check_useable()) {
      ...
      log_warn(m_log, "Game message too short! (kinum=%u)\n", player);
      if (m_log) {
        m_log->dump_contents(Logger::LOG_WARN, mgr->buffer(), mgr->message_len());
      }
      delete in;
      return PROTOCOL_ERROR;
    }
    ...
    if (mgr->is_setup()) {
      bool need_new_id;
      GameMgr *obj = m_game_state.setup_manager_for(player, mgr->setup_uuid(), need_new_id, m_ipaddr, m_id);
      if (!obj) {
      ...
      if (!obj) {
        log_net(m_log, "Received message for nonexistent GameMgr %u (kinum=%u)\n", mgr->gameid(), player);
        // just drop it
      } else {
        try {
...
Server::reason_t GameServer::backend_message(Connection *vault, BackendMessage *in) {
  ...
  if (msg_type & CLASS_MARKER) {
    ...
    if (!mgr) {
      ...
      log_debug(m_log, "Received backend message for now-gone marker manager %u\n", msg->requester());
...
void GameServer::conn_completed(Connection *conn) {
  ...
  if (conn == m_vault) {
    ...
    Hello_BackendMessage *hello = new Hello_BackendMessage(m_ipaddr, m_id, type());
    conn->enqueue(hello);
    TrackGameHello_BackendMessage *gamehello = new TrackGameHello_BackendMessage(m_ipaddr, m_id, m_age_uuid, m_id,
...
Server::reason_t GameServer::conn_timeout(Connection *conn, Server::reason_t why) {
  if (conn == m_vault) {
    ...
    if (m_timed_shutdown) {
      ...
      TrackGameBye_ToBackendMessage *bye = new TrackGameBye_ToBackendMessage(m_ipaddr, m_id, false);
      m_vault->enqueue(bye);
    }
#ifndef STANDALONE
    else {
...
Server::reason_t GameServer::conn_shutdown(Connection *conn, Server::reason_t why) {
  if (conn == m_vault) {
    ...
    for (g_iter = m_game_state.m_age_games.begin(); g_iter != m_game_state.m_age_games.end(); g_iter++) {
      GameMgr *mgr = *g_iter;
  ...
  break;
    ...
    TrackGamePlayerInfo_BackendMessage *track = new TrackGamePlayerInfo_BackendMessage(m_ipaddr, m_id, kinum, false);
...
Server::reason_t GameServer::handle_negotiation(GameConnection *c, const void *keydata, NetworkMessage *in, Logger *log,
    uint32_t &sid) {
  ...
  /* from here is normal processing (after start-up) */
    ...
    if (in->type() == -1) {
      ...
      if (in->type() == kCli2Game_PingRequest) {
        ...
        GamePingMessage *reply = new GamePingMessage(in->buffer(), in->message_len());
        c->enqueue(reply);
        return NO_SHUTDOWN;
      } else {
...
void GameServer::queue_client_connection(GameConnection *conn, NetworkMessage *msg) {
  pthread_mutex_lock(&m_client_queue_mutex);
  m_client_queue->push_back(std::pair<GameConnection*, NetworkMessage*>(conn, msg));
  m_fake_signal = 1;
...
void GameServer::get_queued_connections() {
  ...
  std::deque<std::pair<GameConnection*, NetworkMessage*> > *new_queue, *newconns;
      ...
      if (result == NO_SHUTDOWN) {
        ...
        if (conn->m_read_off < conn->m_read_fill) {
          ...
          if (conn->m_bigbuf) {
            ...
            } while (msg && (conn->m_read_fill > conn->m_read_off) && result == NO_SHUTDOWN);
          }
        } else {
...
Server::reason_t GameServer::handle_join_request(Connection *conn, NetworkMessage *in) {
  GameConnection *gconn = (GameConnection*) conn;
  GameJoinRequest *join = (GameJoinRequest*) in;
  status_code_t result = NO_ERROR;
  ...
  for (t_iter = m_timers->begin(); t_iter != m_timers->end(); t_iter++) {
    ...
    if (timer->type() == GameTimer::CLIENT_JOIN) {
      ...
      if (jt->m_kinum == join->kinum() && !memcmp(jt->m_acct_uuid, join->uuid(), UUID_RAW_LEN)) {
        // it's a match
        if (!jt->cancelled()) {
  ...
  log_info(m_log, "Sending JoinAgeReply (result %u) for kinum %u on fd %d\n", result, join->kinum(), conn->fd());
    ...
    for (c_iter = m_conns.begin(); c_iter != m_conns.end(); c_iter++) {
      ...
      if (c != conn && c != m_vault && c != m_timers) {
        ...
        if (gc->kinum() == join->kinum()) {
          // bleah.
          log_warn(m_log, "Player kinum %u is arriving, but already present\n", join->kinum());
  ...
  if (already_joined) {
    ...
    TrackGamePlayerInfo_BackendMessage *track = new TrackGamePlayerInfo_BackendMessage(m_ipaddr, m_id, join->kinum(), true);
    m_vault->enqueue(track);

#ifndef STANDALONE
    // tell other players this one is here
    PlNetMsgMembersMsg *member_msg = new PlNetMsgMembersMsg(join->kinum());
    member_msg->addMember(join->kinum(), &(gconn->player_name()), &(gconn->plKey()), true);
    member_msg->finalize(false);
    std::list<Connection*>::iterator c_iter;
    for (c_iter = m_conns.begin(); c_iter != m_conns.end(); c_iter++) {
      Connection *c = *c_iter;
