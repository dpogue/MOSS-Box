...
typedef struct {
  ...
  bool (*handle_message)(PropagateBufferMessage *msg, GameState *state, GameServer::GameConnection *conn, Logger *log);
} propagate_handler;
 ...
 */
typedef enum {
  Invalid = 0, VarSync = 1, Marker = 2, BlueSpiral = 3, Heek = 4
} game_type_t;
...
public:
  ...
  virtual bool initialize_game(const GameMgrMessage *msg, kinum_t player, GameServer *server) = 0;
  // this is called when a message is received
  // returns false if the message is not handled, throws truncated_message
  virtual bool got_message(GameMgrMessage *msg, kinum_t player, GameServer *server) = 0;
  ...
  // marker games)
  virtual bool process_backend_message(NetworkMessage *in, GameServer *server) {
    return false;
  }
...
protected:
  ...
  virtual void send_setup_reply(const GameMgrMessage *msg, kinum_t player, GameServer *server);
