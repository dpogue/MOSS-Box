...
public:
  ...
  bool check_ports() {
    if (bind_port > 65535 || bind_port < 1 || track_port > 65535 || track_port < 1) {
  ...
  TrackServiceTypes_BackendMessage* construct_tracking_update(uint32_t id1, uint32_t id2);
protected:
  Logger *m_log;
...
static const char *http_reply = "HTTP/1.0 200 OK\r\nContent-Type: text/html\r\n\r\n";
...
int32_t main(int32_t argc, char *argv[]) {
  ...
  if (!dp->check_log_level() || !dp->check_ports() || !dp->parse_server_types() || !dp->check_auth_log_level()
      || !dp->check_file_log_level() || !dp->check_game_log_level() || !dp->check_gate_log_level()) {
    return_value = 1;
    goto early_shutdown;
  }
  ...
  if (dp->bind_addr_name && dp->bind_addr_name[0] != '\0') {
    const char *result = resolve_hostname(dp->bind_addr_name, &bind_addr.sin_addr.s_addr);
    if (result) {
      log_err(log, "Could not resolve \"%s\": %s\n", dp->bind_addr_name, result);
      return_value = 1;
    }
  } else {
  ...
  if (dp->track_addr_name && dp->track_addr_name[0] != '\0') {
    const char *result = resolve_hostname(dp->track_addr_name, &track_addr.sin_addr.s_addr);
    if (result) {
      log_err(log, "Could not resolve \"%s\": %s\n", dp->track_addr_name, result);
      return_value = 1;
    }
  } else {
  ...
  try {
    ...
    log = new Logger("dispatcher", dp->log_dir ? temp_str : "moss.log", Logger::str_to_level(dp->log_level));
    dp->set_logger(log);
  } catch (const std::bad_alloc&) {
  }
  if (temp_str) {
  ...
  shutdown: if (dp->m_thread_manager) {
  ...
  if (todo[RELOAD]) {
    ...
    if (!s->log()) {
      ...
      try {
        ...
        m_log = new Logger("dispatcher", log_dir ? temp_str : "moss.log", Logger::str_to_level(log_level));
        s->set_logger(m_log);
      } catch (const std::bad_alloc&) {
      }
      if (temp_str) {
    ...
    if ((did_game != m_do_game) || (did_auth != m_do_auth) || (did_file != m_do_file)
        ...
        || (old_ext_addr_name != ext_addr_name
            && strcasecmp(old_ext_addr_name ? old_ext_addr_name : "", ext_addr_name ? ext_addr_name : ""))) {
      log_info(m_log, "Sending new service type available msg ( %s%s%s)\n", m_do_game ? "game " : "",
          m_do_auth ? "auth " : "", m_do_file ? "file " : "");
      server->send_tracking_update(construct_tracking_update(server->id1(), server->id2()));
    }
...
void Dispatcher::add_client_conn(int32_t fd, uint8_t type) {
  ...
  if (type == dp->m_do_auth) {
    ...
    if (!m_auth_log) {
      ...
      try {
        if (dp->log_dir) {
          size_t len = strlen(dp->log_dir) + sizeof("moss_auth.log") + 2;
          temp_str = new char[len];
          snprintf(temp_str, len, "%s/moss_auth.log", dp->log_dir);
        }
        m_auth_log = new Logger("auth", dp->log_dir ? temp_str : "moss_auth.log",
...
#endif
      ret = pthread_create(&tid, &m_thread_attr, serv_main, server);
      if (ret) {
    ...
    if (!m_file_log) {
      ...
      try {
        if (dp->log_dir) {
          size_t len = strlen(dp->log_dir) + sizeof("moss_file.log") + 2;
          temp_str = new char[len];
          snprintf(temp_str, len, "%s/moss_file.log", dp->log_dir);
        }
        m_file_log = new Logger("file", dp->log_dir ? temp_str : "moss_file.log",
    ...
    GameServer::GameConnection *conn = new GameServer::GameConnection(fd, m_log);
    ...
    if (!m_gate_log) {
      ...
      try {
        if (dp->log_dir) {
          size_t len = strlen(dp->log_dir) + sizeof("moss_gatekeeper.log") + 2;
          temp_str = new char[len];
          snprintf(temp_str, len, "%s/moss_gatekeeper.log", dp->log_dir);
        }
        m_gate_log = new Logger("gatekeeper", dp->log_dir ? temp_str : "moss_gatekeeper.log",
    ...
    try {
      ...
      log_err(m_log, "Cannot allocate memory for Gatekeeper server\n");
      log_err(m_log, "Closing connection!\n");
      close(fd);
    }
    if (server) {
    ...
    if (!strncmp(request, "ET /serverstatus/", rsz)
        && (!strncmp(request + rsz, "urulivelive.php HTTP", sizeof("urulivelive.php HTTP") - 1)
            || !strncmp(request + rsz, "moullive.php HTTP", sizeof("moullive.php HTTP") - 1))) {
      ret = write(fd, http_reply, strlen(http_reply));
      if (dp->status_str) {
        ret = write(fd, dp->status_str, dp->status_len);
      } else {
        ret = write(fd, "Welcome to MOSS", sizeof("Welcome to MOSS"));
      }
    } else {
...
int32_t Dispatcher::do_connect() {
  ...
  if (connect(m_track->fd(), (struct sockaddr*) &m_track_addr, sizeof(struct sockaddr_in))) {
...
Dispatcher::~Dispatcher() {
  ...
  for (std::list<SDLDesc*>::iterator iter = m_common_sdl.begin(); iter != m_common_sdl.end(); iter++) {
    SDLDesc *sdl = *iter;
    delete sdl;
  }
