...
#undef HASH_SWAP
    ...
    SHA(inbuf, 28, new_hash);
      ...
      if (login_result.result_code == NO_ERROR) {
        ...
        if (current) {
          ...
          log_debug(m_log, "Account %s has re-logged in, kicking off a "
              "previous login\n", msg->name()->c_str());
          KillClient_BackendMessage *killit = new KillClient_BackendMessage(current->ipaddr(), current->server_id(),
      ...
      if (login_result.result_code == NO_ERROR) {
        reply = new AuthAcctLogin_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(),
            login_result.result_code, login_result.uuid, login_result.is_visitor ? GUEST_CUSTOMER : PAYING_CUSTOMER,
            new UruString(login_result.user_class), buf, buflen, true);
      } else {
        reply = new AuthAcctLogin_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(),
  ...
  case AUTH_KI_VALIDATE:
    {
      ...
      if (m_log && m_log->would_log_at(Logger::LOG_DEBUG)) {
        ...
        log_debug(m_log, "AUTH_KI_VALIDATE request KI %u UUID %s: result %u\n", msg->kinum(), uuid_formatted,
            ki_result);
      }
      AuthKIValidate_FromBackendMessage *reply = new AuthKIValidate_FromBackendMessage(in->get_id1(), in->get_id2(),
          msg->kinum(), ki_result);
      c->enqueue(reply);
  ...
  case AUTH_CHANGE_PASSWORD:
    {
      ...
      snprintf(text_version, 41, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"
          ...
          hash[7], hash[8], hash[9], hash[10], hash[11], hash[12], hash[13], hash[14], hash[15], hash[16], hash[17],
          hash[18], hash[19]);
      ...
      AuthChangePassword_FromBackendMessage *reply = new AuthChangePassword_FromBackendMessage(in->get_id1(),
          in->get_id2(), msg->reqid(), change_result);
      c->enqueue(reply);
    }
    break;
  default:
...
Server::reason_t BackendServer::handle_vault(Connection *c, BackendMessage *in) {
  ...
  case VAULT_PLAYER_CREATE:
    {
      ...
      try {
        try {
          my->C->perform(
              VaultPlayerCreate_Request(msg->acct_uuid(), msg->name()->c_str(), msg->gender()->c_str(), player,
          ...
          if (player.kinum == ERROR_PLAYER_NOT_FOUND) {
            ...
            my->C->perform(
                VaultPlayerCreate_Request(msg->acct_uuid(), msg->name()->c_str(), msg->gender()->c_str(),
                    player, neighbors_list, pinfo));
          } else {
      ...
      if (player.kinum >= MIN_NODEVAL) {
        reply = new VaultPlayerCreate_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), NO_ERROR,
            player.kinum, player.explorer_type, new UruString(player.name), new UruString(player.gender));
#ifndef STANDALONE
        if (neighbors_list != 0) {
  ...
  case VAULT_PLAYER_DELETE:
    {
      ...
      log_at((del_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, "VAULT_PLAYER_DELETE for %u%s\n",
  ...
  case VAULT_FETCHREFS:
    {
      ...
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), refs_buf,
          ref_at, false, true);
      c->enqueue(reply);
    }
    break;
  case VAULT_FINDNODE:
    {
...
#ifdef USE_PQXX
        try {
          my->C->perform(VaultFindNode_Generic(findnode, find_result, find_list, m_log));
        } catch (const pqxx::broken_connection &e) {
          // pretty much fatal -- need to shut down or something
          log_err(m_log, "Connection to DB failed!\n");
          find_result = ERROR_DB_TIMEOUT;
      ...
      log_msgs(m_log, "VAULT_FINDNODE for type %d reqid %u: %u results\n", findnode->type(), msg->reqid(),
          read32(find_buf, 10));
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), find_buf,
          msglen, false, true);
      c->enqueue(reply);
    }
    break;
  case VAULT_FETCH:
    {
      ...
      VaultNodeFetch_FromBackendMessage *reply = new VaultNodeFetch_FromBackendMessage(in->get_id1(), in->get_id2(),
          msg->reqid(), f_result, f_node);
      c->enqueue(reply);
    }
    break;
  case VAULT_SAVENODE:
...
#endif
      log_at((save_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log,
          "VAULT_SAVENODE reqid %u node %u%s\n", msg->reqid(), msg->node_id(),
              ...
              if (getuuid_result == NO_ERROR) {
                ...
                if (to_entity) {
                  ...
                  TrackSDLUpdate_BackendMessage *sdlmsg = new TrackSDLUpdate_BackendMessage(
                      to_entity->ipaddr(), to_entity->server_id(), from_entity->kinum(), in->get_id1(),
                      in->get_id2(), sdl_data + 4, read32(sdl_data, 0),
                      TrackSDLUpdate_BackendMessage::VAULT_SDL_UPDATE);
                  to_entity->conn()->enqueue(sdlmsg);
                }
              }
  ...
  case VAULT_CREATENODE:
    {
      ...
      log_at((c_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, "VAULT_CREATENODE reqid %u node %u%s\n",
          msg->reqid(), c_node, c_result == NO_ERROR ? "" : " FAILED");
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(),
  ...
  case VAULT_ADDREF:
    {
      ...
      log_at((add_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log,
  ...
  c->enqueue(reply);
      ...
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), add_buf,
          10, false, true);
      c->enqueue(reply);
#endif
    }
  ...
  case VAULT_REMOVEREF:
    {
      ...
      log_at((rem_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log,
  ...
  c->enqueue(reply);
      ...
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), rem_buf,
          10, false, true);
      c->enqueue(reply);
#endif
    }
  ...
  case VAULT_INIT_AGE:
    {
      ...
      try {
        my->C->perform(
            VaultCreateAge_Request(msg->age_filename()->c_str(), msg->instance_name()->c_str(),
                msg->user_defined_name()->c_str(), msg->display_name()->c_str(), msg->create_uuid(),
                msg->parent_uuid(), age_node, age_info_node, init_result));
      } catch (const pqxx::in_doubt_error &e) {
      ...
      log_at((init_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, "VAULT_INIT_AGE for %s (%s)%s\n",
          msg->display_name()->c_str(), msg->age_filename()->c_str(), init_result == NO_ERROR ? "" : " failed");
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(),
...
#undef WRITE_STR
    ...
    write32(agelist_buf, age_at, 0);
      ...
      log_at((list_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, "VAULT_AGE_LIST %s (%u entr%s)%s\n",
          msg->age_filename()->c_str(), list_size, (list_size == 1 ? "y" : "ies"),
          list_result == NO_ERROR ? "" : " failed");
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(),
          agelist_buf, 14 + (2464 * list_size), false, true);
      c->enqueue(reply);
    }
    break;
  case VAULT_SENDNODE:
    {
      ...
      try {
        try {
          my->C->perform(VaultSendNode_Request(msg->player(), msg->nodeid(), entity->kinum(), send_result, inboxid));
        } catch (const pqxx::in_doubt_error &e) {
...
#endif
      log_msgs(m_log, "VAULT_SET_AGE_PUBLIC %u to %s%s\n", msg->age_nodeid(), msg->set_public() ? "public" : "private",
  ...
  case VAULT_SCORE_GET:
    {
      ...
      try {
        try {
          my->C->perform(
              VaultGetScore_Request(msg->holder(), msg->score_name(), sget_id, sget_time, sget_type, sget_value,
                  sget_result));
        } catch (const pqxx::in_doubt_error &e) {
      ...
      if (sget_result != NO_ERROR) {
        ...
        reply = new VaultScoreGet_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), sget_result, sget_id,
            msg->holder(), sget_time, sget_type, sget_value, msg->score_name());
      }
      c->enqueue(reply);
    }
    break;
  case VAULT_SCORE_CREATE:
    {
      ...
      try {
        try {
          my->C->perform(
              VaultCreateScore_Request(msg->holder(), msg->score_name(), msg->score_type(), msg->initial_value(),
          ...
          if (scr_result == ERROR_SCORE_EXISTS) {
            ...
            my->C->perform(
                VaultGetScore_Request(msg->holder(), msg->score_name(), scr_id, scr_time, sget_type, sget_value,
                    scr_result));
          }
        }
      } catch (const pqxx::in_doubt_error &e) {
      ...
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), scr_buf,
          18, false, true);
      c->enqueue(reply);
    }
    break;
  case VAULT_SCORE_ADD:
    {
      ...
      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), sadd_buf,
          10, false, true);
      c->enqueue(reply);
    }
    break;
  ...
  case ADMIN_HELLO:
    {
      ...
      if (peer_type == 0) {
        ...
        log_msgs(m_log, "ADMIN_HELLO from %s %08x,%08x\n",
            peer_type == TYPE_AUTH ? "auth" : (peer_type == TYPE_GAME ? "game" : "gatekeeper"), in->get_id1(),
            in->get_id2());
        ConnectionEntity *entity;
        HashKey key(in->get_id1(), in->get_id2());
        if (m_hash_table.find(key) != m_hash_table.end()) {
...
Server::reason_t BackendServer::handle_track(Connection *c, BackendMessage *in) {
  ...
  case TRACK_FIND_SERVICE:
    {
      ...
      if (!got_one) {
        // no server available
        log_warn(m_log, "Telling %08x,%08x there is currently no %s service!\n", in->get_id1(), in->get_id2(),
            msg->wants_file() ? "file" : "auth");
        reply = new TrackFindService_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), msg->reqid2(),
            msg->wants_file());
      } else {
        ...
        if (disp->m_use_fa_hostname) {
          log_msgs(m_log, "Telling %08x,%08x to get %s from %08x,%08x (%s)\n", in->get_id1(), in->get_id2(),
              msg->wants_file() ? "file" : "auth", disp->m_id1, disp->m_id2, disp->m_fa_hostname.c_str());
          reply = new TrackFindService_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), msg->reqid2(),
              msg->wants_file(), disp->m_fa_hostname.c_str());
        } else {
          log_msgs(m_log, "Telling %08x,%08x to get %s from %08x,%08x (%08x)\n", in->get_id1(), in->get_id2(),
              msg->wants_file() ? "file" : "auth", disp->m_id1, disp->m_id2, disp->m_fa_ipaddr);
          reply = new TrackFindService_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), msg->reqid2(),
        ...
        for (w_iter = m_timers->begin(); w_iter != m_timers->end(); w_iter++) {
          ...
          if (!memcmp(w->m_ageuuid, msg->age_uuid(), UUID_RAW_LEN)) {
            ...
            if (m_hash_table.find(key) != m_hash_table.end()) {
              ...
              TrackAgeRequest_FromBackendMessage *none = new TrackAgeRequest_FromBackendMessage(w->m_id1,
                  w->m_id2, w->m_reqid, ERROR_INTERNAL);
  ...
  case TRACK_GAME_HELLO:
    {
      ...
      if (db_result != NO_ERROR) {
        ...
        if (db_result != NO_ERROR) {
          ...
          if (sdlbuf && sdllen > 0) {
            ...
            TrackSDLUpdate_BackendMessage *sdlmsg = new TrackSDLUpdate_BackendMessage(msg->get_id1(),
                msg->get_id2(), 0, m_ipaddr, m_id, sdlbuf, sdllen,
        ...
        if (db_result != NO_ERROR) {
          ...
          if (sdlbuf && sdllen > 0) {
            ...
            TrackSDLUpdate_BackendMessage *sdlmsg = new TrackSDLUpdate_BackendMessage(msg->get_id1(),
                msg->get_id2(), 0, m_ipaddr, m_id, sdlbuf, sdllen, TrackSDLUpdate_BackendMessage::GLOBAL_INIT);
            c->enqueue(sdlmsg);
          }
        }
        if (sdlbuf) {
      ...
      for (w_iter = m_timers->begin(); w_iter != m_timers->end(); w_iter++) {
        ...
        if (!memcmp(w->m_ageuuid, msg->age_uuid(), UUID_RAW_LEN)) {
          ...
          TrackAddPlayer_FromBackendMessage *add = new TrackAddPlayer_FromBackendMessage(in->get_id1(), in->get_id2(),
              w->m_kinum, w->m_name, w->m_acctuuid);
          c->enqueue(add);
          // and reset the timeout just this once
          Waiter *new_w = new Waiter(timeout, this, w->m_id1, w->m_id2, w->m_reqid, w->m_kinum, w->m_name,
  ...
  case TRACK_GAME_BYE:
    {
      ...
      if (m_hash_table.find(game) == m_hash_table.end()) {
        ...
        if (!msg->final()) {
          ...
          TrackGameBye_FromBackendMessage *marker = new TrackGameBye_FromBackendMessage(msg->get_id1(),
              msg->get_id2());
          c->enqueue(marker);
        } else {
  ...
  case TRACK_FIND_GAME:
    // this handles a *client* request for a game server
    {
      ...
      KillClient_BackendMessage::kill_reason_t why = handle_age_request(msg->age_uuid(), msg->filename(), false,
    ...
    // this is an ack
    {
      ...
      if (m_hash_table.find(game) == m_hash_table.end()) {
        ...
        log_msgs(m_log, "TRACK_ADD_PLAYER from %08x,%08x: kinum=%u%s (%u)\n", msg->get_id1(), msg->get_id2(),
            msg->kinum(), msg->result() == NO_ERROR ? "" : " FAILED", msg->result());
          ...
          if (!memcmp(w->m_ageuuid, server->uuid(), UUID_RAW_LEN)) {
            ...
            if (m_hash_table.find(key) != m_hash_table.end()) {
              ...
              log_msgs(m_log, "TRACK_FIND_GAME reply to %08x,%08x for client "
                  "reqid %u (kinum=%u)\n", w->m_id1, w->m_id2, w->m_reqid, w->m_kinum);
              TrackAgeRequest_FromBackendMessage *reply;
              if (msg->result() == NO_ERROR) {
                reply = new TrackAgeRequest_FromBackendMessage(w->m_id1, w->m_id2, w->m_reqid, NO_ERROR,
                    server->uuid(), server->server_id(), w->m_node, server->ipaddr());
                ...
                Waiter *new_w = new Waiter(new_t, this, w->m_id1, w->m_id2, w->m_reqid, w->m_kinum, w->m_name,
                    w->m_acctuuid, w->m_ageuuid, w->m_node);
  ...
  case TRACK_GAME_PLAYERINFO:
    {
      ...
      if (m_hash_table.find(game) == m_hash_table.end()) {
        ...
        log_msgs(m_log, "TRACK_GAME_PLAYERINFO from %08x,%08x: kinum=%u %s\n", msg->get_id1(), msg->get_id2(),
            msg->kinum(), msg->present() ? "here" : "left");
              ...
              if (auth->egg1(auth->ipaddr() == msg->get_id1() && auth->server_id() == msg->get_id2())) {
                ...
                if (egg_status == NO_ERROR && parent != 0) {
                  ...
                  for (iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
                    if (iter->second == auth) {
                      VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(
                          iter->first.id1(), iter->first.id2(), added_buf, 14, false, true);
                      auth->conn()->enqueue(reply);
                      break;
                    }
                  }
          ...
          if (originator->type() != TYPE_GAME) {
            ...
            if (game_servers.size() > 0) {
              ...
              for (game = game_servers.begin(); game != game_servers.end(); game++) {
                TrackMsgForward_BackendMessage *next_msg = new TrackMsgForward_BackendMessage((*game)->ipaddr(),
                    (*game)->server_id(), msg, msg->recips_offset());
  ...
  case MARKER_NEWGAME:
    {
      ...
      if (msg->exists()) {
        ...
        try {
          my->C->perform(
              MarkerGameFind_Request(msg->template_uuid(), game_name, internal_id, game_type, mget_result));
        } catch (const pqxx::in_doubt_error &e) {
        ...
        if (mget_result == NO_ERROR || mget_result == ERROR_NODE_NOT_FOUND) {
          MarkerGetGame_BackendMessage *reply = new MarkerGetGame_BackendMessage(in->get_id1(), in->get_id2(), true,
              msg->requester(), (mget_result == NO_ERROR), internal_id, game_type, msg->template_uuid(),
              &game_name);
          c->enqueue(reply);
        }
        if (mget_result == NO_ERROR) {
          ...
          if (mget_result == NO_ERROR && allmarkers.size() > 0) {
            ...
            log_msgs(m_log, "Sending MARKER_STATE for game %u: %u markers%s\n", internal_id, captured.size(),
                mget_result == NO_ERROR ? "" : " ERROR");
            MarkersCaptured_BackendMessage *creply = new MarkersCaptured_BackendMessage(in->get_id1(),
                in->get_id2(), internal_id, msg->requester(), captured.size());
        ...
        try {
          my->C->perform(
              MarkerGameCreate_Request(msg->player(), msg->name(), internal_id, msg->game_type(), game_uuid,
                  mget_result));
        } catch (const pqxx::in_doubt_error &e) {
  ...
  case MARKER_ADD:
    {
      ...
      try {
        my->C->perform(
            MarkerGameAddMarker_Request(msg->localid(), x, y, z, msg->name(), msg->agename(), marker_num,
                madd_result));
      } catch (const pqxx::in_doubt_error &e) {
  ...
  case MARKER_GAME_RENAME:
    {
      ...
      log_msgs(m_log, "MARKER_GAME_RENAME %sgame %u to %s%s\n",
          rename_result == ERROR_NODE_NOT_FOUND ? "non-existent " : "", msg->localid(), msg->name()->c_str(),
  ...
  case MARKER_CAPTURE:
    {
      ...
      try {
        try {
          my->C->perform(
              MarkerGameCaptureMarker_Request(msg->localid(), msg->player(), msg->number(), msg->value(),
                  cap_result));
        } catch (const pqxx::in_doubt_error &e) {
      ...
      if (leaver->type() == TYPE_GAME) {
        ...
        for (w_iter = m_timers->begin(); w_iter != m_timers->end(); w_iter++) {
          ...
          if (!memcmp(w->m_ageuuid, leaver->uuid(), UUID_RAW_LEN)) {
            KillClient_BackendMessage::kill_reason_t why = handle_age_request(leaver->uuid(), NULL, true, w->m_id1,
                w->m_id2, w->m_reqid);
            if (why == KillClient_BackendMessage::UNKNOWN) {
              // only need one
              break;
          ...
          if (leaver->server_id() != 0 || leaver->ipaddr() != 0) {
            ...
            if (m_hash_table.find(key) != m_hash_table.end()) {
              ...
              if (gameserver->type() == TYPE_GAME) {
                KillClient_BackendMessage *killit = new KillClient_BackendMessage(leaver->ipaddr(),
                    leaver->server_id(), KillClient_BackendMessage::AUTH_DISCONNECT, leaver->kinum());
...
void BackendServer::Waiter::callback() {
  ...
  if (m_server->m_hash_table.find(key) != m_server->m_hash_table.end()) {
    ...
    TrackAgeRequest_FromBackendMessage *none = new TrackAgeRequest_FromBackendMessage(m_id1, m_id2, m_reqid,
        ERROR_NO_RESPONSE);
    entity->conn()->enqueue(none);
  } else {
    // client gone?
...
KillClient_BackendMessage::kill_reason_t BackendServer::handle_age_request(const uint8_t *age_uuid, UruString *filename,
  ...
  if (db_result == ERROR_AGE_NOT_FOUND && filename) {
    ...
    if (*filename == "BahroCave" || *filename == "LiveBahroCaves") {
      ...
      try {
        my->C->perform(
            VaultCreateAge_Request(fname, fname, fname, fname, age_uuid, NULL, age_node, age_info_node, db_result));
      } catch (const pqxx::in_doubt_error &e) {
  ...
  if (db_result != NO_ERROR) {
    TrackAgeRequest_FromBackendMessage *noage = new TrackAgeRequest_FromBackendMessage(user_id1, user_id2, reqid,
        db_result);
  ...
  for (iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
    if (!memcmp(iter->second->uuid(), age_uuid, UUID_RAW_LEN)) {
      ...
      if (entity->in_shutdown()) {
        ...
        TrackAddPlayer_FromBackendMessage *bump = new TrackAddPlayer_FromBackendMessage(iter->first.id1(),
            iter->first.id2(), user->kinum(), user->name(), user->uuid());
    ...
    if (m_dispatchers.size() > 0) {
      ...
      do {
        if (m_dispatchers[m_next_dispatcher]->m_accepting_new_game_servers) {
          ...
          // and set a timeout
          if (!force_new) {
            Waiter *w = new Waiter(timeout, this, user_id1, user_id2, reqid, user->kinum(), user->name(),
                user->uuid(), age_uuid, age_node);
            m_timers->insert(w);
          }
          got_one = true;
          break;
        ...
        if (tell_all) {
          ...
          for (size_t i = 0; i < who.size(); i++) {
            if (who[i] == entity->kinum()) {
              reply = new VaultPassthrough_BackendMessage(key.id1(), key.id2(), changed_buf, changed_len, false,
                  false);
              entity->conn()->enqueue(reply);
              list_size--;
              break;
  ...
  for (std::map<HashKey, ConnectionEntity*>::iterator iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
    ...
    if (entity->type() == TYPE_AUTH) {
      ...
      for (mapiter = range.first; mapiter != range.second; mapiter++) {
        ...
        VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(key.id1(), key.id2(), msgbuf, 10,
            false, false);
        entity->conn()->enqueue(reply);
      }
    }
