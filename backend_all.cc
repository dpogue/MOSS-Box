...
Server::reason_t BackendServer::handle_auth(Connection *c, BackendMessage *in) {
  switch (in->type()) {

  case AUTH_ACCT_LOGIN: {
    AuthAcctLogin_ToBackendMessage *msg = (AuthAcctLogin_ToBackendMessage*) in;
    ...
    try {
      try {
        my->C->perform(AuthAcctLogin_AcctQuery(msg->name()->c_str(), login_result));
...
#undef HASH_SWAP
    ...
    if (login_result.result_code == NO_ERROR) {
      ConnectionEntity *current = find_by_uuid(login_result.uuid, TYPE_AUTH);
      if (current) {
        // punt this one
        log_debug(m_log, "Account %s has re-logged in, kicking off a previous login\n", msg->name()->c_str());
        KillClient_BackendMessage *killit =
            new KillClient_BackendMessage(current->ipaddr(), current->server_id(), KillClient_BackendMessage::NEW_LOGIN);
    ...
    if (login_result.result_code == NO_ERROR) {
      ...
      if (m_hash_table.find(key) == m_hash_table.end()) {
        log_net(m_log, "No ADMIN_HELLO message from peer %08x,%08x type %d\n", in->get_id1(), in->get_id2(), TYPE_AUTH);
      ...
      if (login_result.result_code == NO_ERROR && plist.size() > 0) {
        ...
        for (std::list<AuthAcctLogin_PlayerQuery_Player>::iterator player = plist.begin(); player != plist.end(); player++) {
          write32(buf, buflen, Auth2Cli_AcctPlayerInfo);
          buflen += 2;
          write32(buf, buflen, msg->reqid());
          buflen += 4;
          write32(buf, buflen, player->kinum);
          buflen += 4;
          memcpy(buf + buflen, player->name.get_str(true, true, false), player->name.send_len(true, true, false));
          buflen += player->name.send_len(true, true, false);
          memcpy(buf + buflen, player->gender.get_str(true, true, false), player->gender.send_len(true, true, false));
    ...
    if (m_log && m_log->would_log_at(Logger::LOG_MSGS)) {
      ...
      log_msgs(m_log, "AUTH_ACCT_LOGIN received for %s: "
          "result %u UUID %s\n", msg->name()->c_str(), login_result.result_code, uuid_formatted);
    }
    if (login_result.result_code == NO_ERROR) {
      reply = new AuthAcctLogin_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), login_result.result_code,
          login_result.uuid, login_result.is_visitor ? GUEST_CUSTOMER : PAYING_CUSTOMER, new UruString(login_result.user_class),
          buf, buflen, true);
    } else {
      reply = new AuthAcctLogin_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), login_result.result_code);
    }
  ...
  case AUTH_KI_VALIDATE: {
    AuthKIValidate_ToBackendMessage *msg = (AuthKIValidate_ToBackendMessage*) in;
...
#ifdef USE_PQXX
    try {
      my->C->perform(AuthValidateKI(msg->acct_uuid(), msg->kinum(), ki_result, player_name));
    } catch (const pqxx::broken_connection &e) {
      // pretty much fatal -- need to shut down or something
      log_err(m_log, "Connection to DB failed!\n");
    ...
    if (m_log && m_log->would_log_at(Logger::LOG_DEBUG)) {
      ...
      log_debug(m_log, "AUTH_KI_VALIDATE request KI %u UUID %s: result %u\n", msg->kinum(), uuid_formatted, ki_result);
    }
    AuthKIValidate_FromBackendMessage *reply = new AuthKIValidate_FromBackendMessage(in->get_id1(), in->get_id2(), msg->kinum(),
        ki_result);
    c->enqueue(reply);
    ...
    if (ki_result == NO_ERROR) {
      ...
      try {
        try {
          ...
          log_warn(m_log, "in_doubt in SetPlayerConnected; retrying\n");
          my->C->perform(SetPlayerConnected(msg->kinum(), spc));
        }
      } catch (const pqxx::in_doubt_error &e) {
  ...
  case AUTH_PLAYER_LOGOUT: {
    AuthPlayerLogout_BackendMessage *msg = (AuthPlayerLogout_BackendMessage*) in;
  ...
  case AUTH_CHANGE_PASSWORD: {
    ...
    snprintf(text_version, sizeof(text_version),
    ...
    try {
      try {
        my->C->perform(AuthChangePassword(msg->acct_uuid(), msg->name()->c_str(), text_version, change_result));
      } catch (const pqxx::in_doubt_error &e) {
        // we can check the DB to see if the new hash is present
        log_warn(m_log, "in_doubt in AuthChangePassword; checking result\n");
        my->C->perform(AuthVerifyPassword(msg->acct_uuid(), msg->name()->c_str(), text_version, change_result));
    ...
    if (change_result == ERROR_ACCT_NOT_FOUND) {
      ...
      log_msgs(m_log, "AUTH_CHANGE_PASSWORD for %s%s\n", msg->name()->c_str(), change_result == NO_ERROR ? "" : " FAILED");
    }

    AuthChangePassword_FromBackendMessage *reply = new AuthChangePassword_FromBackendMessage(in->get_id1(), in->get_id2(),
        msg->reqid(), change_result);
    c->enqueue(reply);
  }
    break;
  default:
 ...
 *@{
 */
Server::reason_t BackendServer::handle_vault(Connection *c, BackendMessage *in) {

  switch (in->type()) {

  case VAULT_PLAYER_CREATE: {
    VaultPlayerCreate_ToBackendMessage *msg = (VaultPlayerCreate_ToBackendMessage*) in;
    ...
    try {
      try {
        my->C->perform(
            VaultPlayerCreate_Request(msg->acct_uuid(), msg->name()->c_str(), msg->gender()->c_str(), player, neighbors_list,
                pinfo));
      } catch (const pqxx::in_doubt_error &e) {
        log_warn(m_log, "in_doubt in VaultPlayerCreate; attempting to recover\n");
        // let us see if the create happened
        my->C->perform(VaultPlayerRequest_Verify(msg->acct_uuid(), msg->name()->c_str(), player));
      ...
      log_warn(m_log, "in_doubt again in VaultPlayerCreate; "
          "is something badly wrong with the DB?\n");
      KillClient_BackendMessage *killit = new KillClient_BackendMessage(in->get_id1(), in->get_id2(),
    ...
    log_msgs(m_log, "VAULT_PLAYER_CREATE for %s (%s) %s %u\n", msg->name()->c_str(), msg->gender()->c_str(),
        player.kinum >= MIN_NODEVAL ? "-> KI" : "failed:", player.kinum);
    VaultPlayerCreate_FromBackendMessage *reply;
    if (player.kinum >= MIN_NODEVAL) {
      reply = new VaultPlayerCreate_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), NO_ERROR, player.kinum,
          player.explorer_type, new UruString(player.name), new UruString(player.gender));
#ifndef STANDALONE
      if (neighbors_list != 0) {
      ...
      reply = new VaultPlayerCreate_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(),
  ...
  case VAULT_PLAYER_DELETE: {
    VaultPlayerDelete_ToBackendMessage *msg = (VaultPlayerDelete_ToBackendMessage*) in;
...
#ifdef USE_PQXX
    try {
      my->C->perform(VaultPlayerDelete_Request(msg->kinum(), del_result, notifies, pinfo));
    } catch (const pqxx::in_doubt_error &e) {
    ...
    log_at_where((del_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log,
    LOGGER_WHERE, "VAULT_PLAYER_DELETE for %u%s\n", msg->kinum(), del_result == NO_ERROR ? "" : " failed");
    VaultPlayerDelete_FromBackendMessage *reply = new VaultPlayerDelete_FromBackendMessage(in->get_id1(), in->get_id2(),
        msg->reqid(), del_result == ERROR_NODE_NOT_FOUND ? NO_ERROR : del_result);
#ifndef STANDALONE
  ...
  case VAULT_FETCHREFS: {
    VaultFetchRefs_ToBackendMessage *msg = (VaultFetchRefs_ToBackendMessage*) in;
...
#ifdef USE_PQXX
    try {
      my->C->perform(VaultFetchRefs_Request(msg->node_id(), refs_result, refs_list));
    } catch (const pqxx::broken_connection &e) {
      // pretty much fatal -- need to shut down or something
      log_err(m_log, "Connection to DB failed!\n");
    ...
    write16(refs_buf, 0, Auth2Cli_VaultNodeRefsFetched);
    write32(refs_buf, 2, msg->reqid());
    write32(refs_buf, 6, refs_result);
    write32(refs_buf, 10, refs_list.size());
    uint32_t ref_at = 14;
    for (std::vector<VaultFetchRefs_VaultRef>::const_iterator ref_el = refs_list.begin(); ref_el != refs_list.end(); ref_el++) {
    ...
    VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), refs_buf, ref_at,
        false, true);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_FINDNODE: {
    ...
    if (findnode->bitfield1() == 0x00001080 && findnode->bitfield2() == 0 && findnode->type() == VaultNode::PlayerInfoNode) {
      // use original, simple code
      uint32_t find_val = 0;
      uint32_t node_id = findnode->num_val(UInt32_1);
#ifdef USE_PQXX
      try {
        my->C->perform(VaultFindNode_Request(node_id, find_result, find_val));
      } catch (const pqxx::broken_connection &e) {
        // pretty much fatal -- need to shut down or something
        log_err(m_log, "Connection to DB failed!\n");
...
#ifdef USE_PQXX
      try {
        my->C->perform(VaultFindNode_Generic(findnode, find_result, find_list, m_log, true));
      } catch (const pqxx::broken_connection &e) {
        // pretty much fatal -- need to shut down or something
        log_err(m_log, "Connection to DB failed!\n");
        find_result = ERROR_DB_TIMEOUT;
    ...
    write16(find_buf, 0, Auth2Cli_VaultNodeFindReply);
    ...
    log_msgs(m_log, "VAULT_FINDNODE for type <%d>\"%s\" reqid %u: %u results\n", findnode->type(),
        VaultNode::tablename_for_type(findnode->type()), msg->reqid(), read32(find_buf, 10));
    VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), find_buf, msglen,
        false, true);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_FETCH: {
    VaultNodeFetch_ToBackendMessage *msg = (VaultNodeFetch_ToBackendMessage*) in;
...
#ifdef USE_PQXX
    try {
      my->C->perform(VaultFetchNode_Request(msg->node_id(), f_result, *f_node, m_log));
    } catch (const pqxx::broken_connection &e) {
      // pretty much fatal -- need to shut down or something
      log_err(m_log, "Connection to DB failed!\n");
    ...
    if (f_result != NO_ERROR) {
      ...
      log_msgs(m_log, "VAULT_FETCH reqid %u node %u -> node of type <%d>\"%s\"\n",
          msg->reqid(),
          msg->node_id(),
          f_node->type(),
          VaultNode::tablename_for_type(f_node->type()));
    }
    VaultNodeFetch_FromBackendMessage *reply = new VaultNodeFetch_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(),
        f_result, f_node);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_SAVENODE: {
    ...
    try {
      try {
        my->C->perform(VaultSaveNode_Request(msg->node_id(), msg->data(), save_result, m_log));
      } catch (const pqxx::in_doubt_error &e) {
        // just retry, it does not hurt to save with the same data
        my->C->perform(VaultSaveNode_Request(msg->node_id(), msg->data(), save_result, m_log));
      }
    } catch (const pqxx::in_doubt_error &e) {
      log_warn(m_log, "in_doubt again in VaultSaveNode; is something badly wrong with the DB?\n");
    } catch (const pqxx::broken_connection &e) {
      // pretty much fatal -- need to shut down or something
      log_err(m_log, "Connection to DB failed!\n");
    ...
    log_at_where((save_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, LOGGER_WHERE,
        "VAULT_SAVENODE reqid %u node %u%" /* " -> type <%d>\"%s\""*/ "\n",
        msg->reqid(),
        msg->node_id(),
    ...
    write16(save_buf, 0, Auth2Cli_VaultSaveNodeReply);
    write32(save_buf, 2, msg->reqid());
    write32(save_buf, 6, save_result);
    reply = new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), save_buf, 10, false, true);
    c->enqueue(reply);
    if (save_result == NO_ERROR) {
#ifdef STANDALONE
      // for now just bounce it back to the original (only!) client
      uint8_t *changed_buf = new uint8_t[22];
      write16(changed_buf, 0, Auth2Cli_VaultNodeChanged);
      write32(changed_buf, 2, msg->node_id());
      memcpy(changed_buf+6, msg->requuid(), 16);
      reply =
          new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), changed_buf, 22, false, true);
      c->enqueue(reply);
#else
      ...
      if (msg->data()->type() == VaultNode::SDLNode) {
        ...
        if (sdl_data && read32(sdl_data, 0) > 0) {
          ...
          HashKey key(in->get_id1(), in->get_id2());
          if (m_hash_table.find(key) == m_hash_table.end()) {
            log_warn(m_log, "VAULT_SAVENODE for unknown connection ID %08x,%08x!\n",
                in->get_id1(), in->get_id2());
          } else {
            ...
            if (m_hash_table.find(key2) == m_hash_table.end()) {
              ...
              // maybe other times
                ...
                log_debug(m_log, "Forwarding saved vault SDL to player's game server id %08x,%08x\n",
                    game_id1, game_id2);
            ...
            if (getuuid_result == ERROR_NODE_NOT_FOUND) {
              log_warn(m_log, "Vault SDL was saved for node %u, which has no age??\n",
                  msg->node_id());
            }
            if (getuuid_result == NO_ERROR) {
              ...
              if (to_entity) {
                // yep, forward it on
                log_debug(m_log, "Forwarding saved vault SDL to corresponding game server id %08x,%08x\n",
                    to_entity->ipaddr(), to_entity->server_id());
                TrackSDLUpdate_BackendMessage *sdlmsg = new TrackSDLUpdate_BackendMessage(
                    to_entity->ipaddr(),
                    to_entity->server_id(),
                    from_entity->kinum(),
                    in->get_id1(), in->get_id2(),
  ...
  case VAULT_CREATENODE: {
    ...
    HashKey key(in->get_id1(), in->get_id2());
    if (m_hash_table.find(key) == m_hash_table.end()) {
      log_err(m_log, "VAULT_CREATENODE for connection ID %08x,%08x but I don't know about that connection!\n",
          in->get_id1(), in->get_id2());
      KillClient_BackendMessage *killit =
          new KillClient_BackendMessage(in->get_id1(), in->get_id2(), KillClient_BackendMessage::NO_STATE);
      c->enqueue(killit);
      break;
    }
    ConnectionEntity *entity = m_hash_table[key];
    ...
    try {
      try {
        my->C->perform(VaultCreateNode_Request(msg->data(), entity->uuid(), entity->kinum(), c_node, m_log));
      } catch (const pqxx::in_doubt_error &e) {
      ...
      log_warn(m_log, "in_doubt again in VaultCreateNode; is something badly wrong with the DB?\n");
    ...
    write16(create_buf, 0, Auth2Cli_VaultNodeCreated);
    write32(create_buf, 2, msg->reqid());
    write32(create_buf, 6, c_result);
    write32(create_buf, 10, c_node);
    log_at_where((c_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, LOGGER_WHERE,
        "VAULT_CREATENODE reqid %u node %u%s\n",
        msg->reqid(),
        c_node,
        c_result == NO_ERROR ? "" : " FAILED");
    VaultPassthrough_BackendMessage *reply =
        new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), create_buf, 14, false, true);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_ADDREF: {
    ...
    try {
      try {
        my->C->perform(VaultAddRef_Request(msg->parent(), msg->child(), msg->owner(), add_result));
      } catch (const pqxx::in_doubt_error &e) {
      ...
      log_warn(m_log, "in_doubt again in VaultAddRef; is something badly wrong with the DB?\n");
      KillClient_BackendMessage *killit =
          new KillClient_BackendMessage(in->get_id1(), in->get_id2(), KillClient_BackendMessage::IN_DOUBT);
      c->enqueue(killit);
      break;
    } catch (const pqxx::broken_connection &e) {
      // pretty much fatal -- need to shut down or something
    ...
    log_at_where((add_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, LOGGER_WHERE,
        "VAULT_ADDREF reqid %u add %u->%u (%u)%s\n",
        msg->reqid(),
        msg->parent(),
        msg->child(),
        msg->owner(),
  ...
  write16(added_buf, 0, Auth2Cli_VaultNodeAdded);
  ...
  c->enqueue(reply);
#else
      propagate_add_to_interested(msg->parent(), msg->child(), msg->owner(), true);
#endif
    }
    uint8_t *add_buf = new uint8_t[10];
    write16(add_buf, 0, Auth2Cli_VaultAddNodeReply);
    write32(add_buf, 2, msg->reqid());
    write32(add_buf, 6, add_result);
    VaultPassthrough_BackendMessage *reply =
        new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), add_buf, 10, false, true);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_REMOVEREF: {
    ...
    try {
      try {
        my->C->perform(VaultRemoveRef_Request(msg->parent(), msg->child(), removed));
      } catch (const pqxx::in_doubt_error &e) {
    ...
    log_at_where((rem_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, LOGGER_WHERE,
        "VAULT_REMOVEREF reqid %u remove %u->%u%s\n", msg->reqid(), msg->parent(), msg->child(),
        rem_result == NO_ERROR ? "" : " FAILED");
    if (rem_result == NO_ERROR && removed > 0) {
#ifdef STANDALONE
  // for now just bounce it back to the original (only!) client
  uint8_t *remd_buf = new uint8_t[10];
  write16(remd_buf, 0, Auth2Cli_VaultNodeRemoved);
  ...
  c->enqueue(reply);
    ...
    write16(rem_buf, 0, Auth2Cli_VaultRemoveNodeReply);
    write32(rem_buf, 2, msg->reqid());
    write32(rem_buf, 6, rem_result);
    VaultPassthrough_BackendMessage *reply =
        new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), rem_buf, 10, false, true);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_INIT_AGE: {
    ...
    try {
      my->C->perform(
          VaultCreateAge_Request(
              msg->age_filename()->c_str(),
    ...
    write16(initage_buf, 0, Auth2Cli_VaultInitAgeReply);
    ...
    log_at_where((init_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, LOGGER_WHERE,
        "VAULT_INIT_AGE for %s (%s)%s\n", msg->display_name()->c_str(), msg->age_filename()->c_str(),
        init_result == NO_ERROR ? "" : " failed");
    VaultPassthrough_BackendMessage *reply =
        new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), initage_buf, 18, false, true);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_AGE_LIST: {
    ...
    write16(agelist_buf, 0, Auth2Cli_PublicAgeList);
    ...
    if (list_result == NO_ERROR) {
      ...
      uint32_t age_at = 14;
      for (std::vector<VaultAgeList_AgeInfo>::iterator age_el = age_list.begin(); age_el != age_list.end(); age_el++) {
...
#undef WRITE_STR
    ...
    log_at_where((list_result == NO_ERROR ? Logger::LOG_MSGS : Logger::LOG_WARN), m_log, LOGGER_WHERE,
        "VAULT_AGE_LIST %s (%u entr%s)%s\n",
        msg->age_filename()->c_str(),
        list_size, (list_size == 1 ? "y" : "ies"),
        list_result == NO_ERROR ? "" : " failed");
    VaultPassthrough_BackendMessage *reply =
        new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), agelist_buf, 14 + (2464 * list_size), false, true);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_SET_AGE_PUBLIC: {
    VaultSetAgePublic_BackendMessage *msg = (VaultSetAgePublic_BackendMessage*) in;

    status_code_t public_result = ERROR_INTERNAL;
#ifdef USE_PQXX
    try {
      try {
        my->C->perform(VaultSetAgePublic_Request(msg->age_nodeid(), msg->set_public(), public_result));
      } catch (const pqxx::in_doubt_error &e) {
  ...
  VaultPassthrough_BackendMessage *reply;
  reply = new VaultPassthrough_BackendMessage(
                in->get_id1(),
                in->get_id2(),
                changed_buf, 22,
                false, true);
  ...
  case VAULT_SCORE_GET: {
    VaultScoreGet_ToBackendMessage *msg = (VaultScoreGet_ToBackendMessage*) in;
    ...
    try {
      try {
        my->C->perform(
            VaultGetScore_Request(msg->holder(), msg->score_name(), sget_id, sget_time, sget_type, sget_value, sget_result));
      } catch (const pqxx::in_doubt_error &e) {
      ...
      log_warn(m_log, "in_doubt again in VaultGetScore; is something badly wrong with the DB?\n");
    } catch (const pqxx::broken_connection &e) {
      // pretty much fatal -- need to shut down or something
      log_err(m_log, "Connection to DB failed!\n");
    ...
    log_msgs(m_log, "VAULT_SCORE_GET %u's %s%s\n",
        ...
        sget_result == NO_ERROR ? "" : (sget_result == ERROR_NO_SCORE ? " (none)" : " failed"));

    VaultScoreGet_FromBackendMessage *reply;
    if (sget_result != NO_ERROR) {
      reply = new VaultScoreGet_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), sget_result);
    } else {
      reply = new VaultScoreGet_FromBackendMessage(in->get_id1(), in->get_id2(),
          msg->reqid(), sget_result, sget_id, msg->holder(), sget_time, sget_type, sget_value, msg->score_name());
    }
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_SCORE_CREATE: {
    VaultScoreCreate_BackendMessage *msg = (VaultScoreCreate_BackendMessage*) in;
    ...
    try {
      try {
        my->C->perform(
            VaultCreateScore_Request(
                msg->holder(),
                msg->score_name(),
                msg->score_type(),
                msg->initial_value(),
                scr_id, scr_time, scr_result));
      } catch (const pqxx::in_doubt_error &e) {
        ...
        if (scr_result == ERROR_SCORE_EXISTS) {
          ...
          my->C->perform(
              VaultGetScore_Request(msg->holder(), msg->score_name(), scr_id, scr_time, sget_type, sget_value, scr_result));
        }
      }
    } catch (const pqxx::in_doubt_error &e) {
      log_warn(m_log, "in_doubt again in VaultCreateScore; is something badly wrong with the DB?\n");
    } catch (const pqxx::broken_connection &e) {
      // pretty much fatal -- need to shut down or something
      log_err(m_log, "Connection to DB failed!\n");
...
#endif
    if (scr_result == ERROR_BAD_SCORE_TYPE || scr_result == ERROR_SCORE_EXISTS) {
    ...
    write16(scr_buf, 0, Auth2Cli_ScoreCreateReply);
    write32(scr_buf, 2, msg->reqid());
    write32(scr_buf, 6, scr_result);
    write32(scr_buf, 10, scr_id);
    write32(scr_buf, 14, scr_time);
    VaultPassthrough_BackendMessage *reply =
        new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), scr_buf, 18, false, true);
    c->enqueue(reply);
  }
    break;

  case VAULT_SCORE_ADD: {
    VaultScoreAddPoints_BackendMessage *msg = (VaultScoreAddPoints_BackendMessage*) in;

    status_code_t sadd_result = ERROR_INTERNAL;
#ifdef USE_PQXX
    try {
      my->C->perform(VaultAddToScore_Request(msg->score_id(), msg->delta(), sadd_result));
    } catch (const pqxx::in_doubt_error &e) {
    ...
    write16(sadd_buf, 0, Auth2Cli_ScoreAddPointsReply);
    write32(sadd_buf, 2, msg->reqid());
    write32(sadd_buf, 6, sadd_result);
    VaultPassthrough_BackendMessage *reply =
        new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), sadd_buf, 10, false, true);
    c->enqueue(reply);
  }
    break;

  case VAULT_SCORE_XFER: {
    VaultScoreXferPoints_BackendMessage *msg = (VaultScoreXferPoints_BackendMessage*) in;

    status_code_t sxfer_result = ERROR_INTERNAL;
#ifdef USE_PQXX
    try {
      my->C->perform(VaultTransferScore_Request(msg->score_id(), msg->dest_id(), msg->delta(), sxfer_result));
    } catch (const pqxx::in_doubt_error &e) {
    ...
    write16(sxfer_buf, 0, Auth2Cli_ScoreTransferPointsReply);
    write32(sxfer_buf, 2, msg->reqid());
    write32(sxfer_buf, 6, sxfer_result);
    VaultPassthrough_BackendMessage *reply =
        new VaultPassthrough_BackendMessage(in->get_id1(), in->get_id2(), sxfer_buf, 10, false, true);
    c->enqueue(reply);
  }
    break;
  ...
  case VAULT_PASSTHRU: {
    VaultPassthrough_BackendMessage *msg = (VaultPassthrough_BackendMessage*) in;
...
Server::reason_t BackendServer::handle_admin(Connection *c, BackendMessage *in) {
  switch (in->type()) {
  case ADMIN_HELLO: {
    ...
    if (peer_type == 0) {
      ...
      for (std::vector<DispatcherInfo*>::iterator iter = m_dispatchers.begin(); iter != m_dispatchers.end(); iter++) {
        DispatcherInfo *disp = *iter;
        if (disp->m_id1 == in->get_id1() && disp->m_id2 == in->get_id2()) {
          log_net(m_log, "Extra ADMIN_HELLO message from peer %08x,%08x type %d\n",
      ...
      // here, it's a new one
      DispatcherInfo *d = new DispatcherInfo(in->get_id1(), in->get_id2(), c);
      m_dispatchers.push_back(d);
    } else {
      // auth or game
      log_msgs(m_log, "ADMIN_HELLO from %s %08x,%08x\n",
          peer_type == TYPE_AUTH ? "auth" : (peer_type == TYPE_GAME ? "game" : "gatekeeper"),
              in->get_id1(),
              in->get_id2());
      ConnectionEntity *entity;
      HashKey key(in->get_id1(), in->get_id2());
...
Server::reason_t BackendServer::handle_track(Connection *c, BackendMessage *in) {
  switch (in->type()) {
  ...
  case TRACK_SERVICE_TYPES: {
    TrackServiceTypes_BackendMessage *msg = (TrackServiceTypes_BackendMessage*) in;
    log_debug(m_log, "TRACK_SERVICE_TYPES: %08x,%08x accepts%s%s%s\n",
        in->get_id1(),
        in->get_id2(),
        msg->has_auth() ? " auth" : "", msg->has_file() ? " file" : "",
        msg->has_game() ? " game" : ((msg->has_file() || msg->has_auth()) ? "" : " -none-"));
  ...
  case TRACK_FIND_SERVICE: {
    TrackFindService_ToBackendMessage *msg = (TrackFindService_ToBackendMessage*) in;
    ...
    if (!got_one) {
      // no server available
      log_warn(m_log, "TRACK_FIND_SERVICE: Telling %08x,%08x there is currently no %s service!\n",
          in->get_id1(), in->get_id2(),
          msg->wants_file() ? "file" : "auth");
      reply = new TrackFindService_FromBackendMessage(in->get_id1(), in->get_id2(), msg->reqid(), msg->reqid2(), msg->wants_file());
    } else {
      ...
      if (disp->m_use_fa_hostname) {
        log_msgs(m_log, "Telling %08x,%08x to get %s from %08x,%08x (%s:%d)\n", in->get_id1(), in->get_id2(),
            msg->wants_file() ? "file" : "auth", disp->m_id1, disp->m_id2, disp->m_fa_hostname.c_str(), disp->m_fa_ipport);
        reply = new TrackFindService_FromBackendMessage(
            in->get_id1(), in->get_id2(),
            msg->reqid(),
            msg->reqid2(),
            msg->wants_file(),
            disp->m_fa_hostname.c_str(),
            disp->m_fa_ipport);
      } else {
        if (inaddr_c_str(addrbuf, sizeof(addrbuf), disp->m_fa_ipaddr, disp->m_fa_ipport, -1))
          strncpy(addrbuf, "XXX", sizeof(addrbuf));
        log_msgs(m_log, "Telling %08x,%08x to get %s from %08x,%08x (%s)\n", in->get_id1(), in->get_id2(),
            msg->wants_file() ? "file" : "auth", disp->m_id1, disp->m_id2, addrbuf);
        reply = new TrackFindService_FromBackendMessage(
            in->get_id1(), in->get_id2(),
            msg->reqid(),
            msg->reqid2(),
  ...
  case TRACK_START_GAME: {
    TrackStartAge_ToBackendMessage *msg = (TrackStartAge_ToBackendMessage*) in;
      ...
      for (w_iter = m_timers->begin(); w_iter != m_timers->end(); w_iter++) {
        ...
        if (!memcmp(w->m_ageuuid, msg->age_uuid(), UUID_RAW_LEN)) {
          ...
          if (m_hash_table.find(key) != m_hash_table.end()) {
            ...
            log_warn(m_log, "Telling client connection %08x,%08x (kinum=%u) we cannot start a game server\n",
                w->m_id1, w->m_id2, w->m_kinum);
            TrackAgeRequest_FromBackendMessage *none =
                new TrackAgeRequest_FromBackendMessage(w->m_id1, w->m_id2, w->m_reqid, ERROR_INTERNAL);
            entity->conn()->enqueue(none);
          } else {
            // client gone?
            log_debug(m_log, "Game server request failed for unknown connection %08x,%08x (kinum=%u)\n",
                w->m_id1, w->m_id2, w->m_kinum);
          }
          w->cancel();
        }
  ...
  case TRACK_GAME_HELLO: {
    TrackGameHello_BackendMessage *msg = (TrackGameHello_BackendMessage*) in;
...
#ifdef USE_PQXX
    try {
      my->C->perform(VaultGetAgeByUUID(msg->age_uuid(), age_node, age_info, age_fname, db_result));
    } catch (const pqxx::in_doubt_error &e) {
    ...
    if (db_result != NO_ERROR) {
...
#ifdef USE_PQXX
      try {
        my->C->perform(GetVaultSDL(age_info, age_fname, &sdlbuf, sdllen, db_result));
      } catch (const pqxx::in_doubt_error &e) {
      ...
      if (db_result != NO_ERROR) {
        ...
        if (sdlbuf && sdllen > 0) {
          ...
          TrackSDLUpdate_BackendMessage *sdlmsg =
              new TrackSDLUpdate_BackendMessage(
                  msg->get_id1(),
                  msg->get_id2(),
                  0,
                  m_ipaddr,
    ...
    for (w_iter = m_timers->begin(); w_iter != m_timers->end(); w_iter++) {
      ...
      if (!memcmp(w->m_ageuuid, msg->age_uuid(), UUID_RAW_LEN)) {
        ...
        TrackAddPlayer_FromBackendMessage *add =
            new TrackAddPlayer_FromBackendMessage(in->get_id1(), in->get_id2(), w->m_kinum, w->m_name, w->m_acctuuid);
        c->enqueue(add);
        // and reset the timeout just this once
        Waiter *new_w =
            new Waiter(timeout, this, w->m_id1, w->m_id2, w->m_reqid, w->m_kinum, w->m_name, w->m_acctuuid, w->m_ageuuid, w->m_node);
        w->cancel();
    ...
    for (std::list<Waiter*>::iterator n_iter = new_waiters.begin(); n_iter != new_waiters.end(); n_iter++) {
  ...
  case TRACK_GAME_BYE: {
    ...
    if (m_hash_table.find(game) == m_hash_table.end()) {
      // not found!
      log_err(m_log, "Dropping TRACK_GAME_BYE from peer %08x,%08x which has not registered as a game server\n",
          msg->get_id1(), msg->get_id2());
      ...
      if (!msg->final()) {
        ...
        TrackGameBye_FromBackendMessage *marker = new TrackGameBye_FromBackendMessage(msg->get_id1(), msg->get_id2());
        c->enqueue(marker);
      } else {
  ...
  {
    TrackAgeRequest_ToBackendMessage *msg = (TrackAgeRequest_ToBackendMessage*) in;
#ifndef STANDALONE
    ...
    KillClient_BackendMessage::kill_reason_t why = handle_age_request(msg->age_uuid(), msg->filename(), false, in->get_id1(),
        in->get_id2(), msg->reqid());
    if (why != KillClient_BackendMessage::UNKNOWN) {
      KillClient_BackendMessage *killit = new KillClient_BackendMessage(in->get_id1(), in->get_id2(), why);
      c->enqueue(killit);
    }
  }
    break;
  ...
  case TRACK_ADD_PLAYER:
    // this is an ack
  {
    TrackAddPlayer_ToBackendMessage *msg = (TrackAddPlayer_ToBackendMessage*) in;
      ...
      for (w_iter = m_timers->begin(); w_iter != m_timers->end(); w_iter++) {
        ...
        if (!memcmp(w->m_ageuuid, server->uuid(), UUID_RAW_LEN)) {
          ...
          if (m_hash_table.find(key) != m_hash_table.end()) {
            ...
            char addr[INET_ADDRSTRLEN + sizeof(":12345")];
            inaddr_c_str(addr, sizeof(addr), htonl(server->ipaddr()), 0, 0);

            log_msgs(m_log, "TRACK_FIND_GAME reply to %08x,%08x for client reqid %u (kinum=%u) server %s\n", w->m_id1, w->m_id2,
                w->m_reqid, w->m_kinum, addr);
            TrackAgeRequest_FromBackendMessage *reply;
            if (msg->result() == NO_ERROR) {
              reply =
                  new TrackAgeRequest_FromBackendMessage(
                      w->m_id1,
                      w->m_id2,
                      w->m_reqid,
                      NO_ERROR,
              ...
              Waiter *new_w =
                  new Waiter(new_t, this, w->m_id1, w->m_id2, w->m_reqid, w->m_kinum, w->m_name, w->m_acctuuid, w->m_ageuuid, w->m_node);
              // this insert is safe because we are going to stop iterating
              // over the timers list now
              m_timers->insert(new_w);
            } else {
              reply = new TrackAgeRequest_FromBackendMessage(w->m_id1, w->m_id2, w->m_reqid, msg->result());
              entity->conn()->enqueue(reply);
            }
          } else {
            // client timed out, I guess
            log_debug(m_log, "Game server replied about unknown connection %08x,%08x (kinum=%u)\n",
                w->m_id1, w->m_id2, w->m_kinum);
          }
          w->cancel();
          break;
  ...
  case TRACK_GAME_PLAYERINFO: {
    TrackGamePlayerInfo_BackendMessage *msg = (TrackGamePlayerInfo_BackendMessage*) in;

    HashKey game(msg->get_id1(), msg->get_id2());
    if (m_hash_table.find(game) == m_hash_table.end()) {
      // not found
      log_err(m_log, "Dropping TRACK_GAME_PLAYERINFO from peer %08x,%08x which has not registered as a game server\n",
          msg->get_id1(), msg->get_id2());
      // just forget the message
    } else {
      log_msgs(m_log, "TRACK_GAME_PLAYERINFO from %08x,%08x: kinum=%u %s\n",
          msg->get_id1(), msg->get_id2(), msg->kinum(), msg->present() ? "here" : "left");
      ...
      if (auth) {
        ...
        if (msg->present()) {
          if (!(m_egg_mask & (1 << 1))) {
            if (auth->egg1(auth->ipaddr() == msg->get_id1() && auth->server_id() == msg->get_id2())) {
...
#ifdef USE_PQXX
              try {
                my->C->perform(Egg1(msg->kinum(), parent, child, egg_status));
              } catch (const pqxx::in_doubt_error &e) {
              ...
              if (egg_status == NO_ERROR && parent != 0) {
                ...
                for (iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
                  if (iter->second == auth) {
                    VaultPassthrough_BackendMessage *reply =
                        new VaultPassthrough_BackendMessage(iter->first.id1(), iter->first.id2(), added_buf, 14, false, true);
    ...
    if (recip_offset < msg_len) {
      ...
      if (m_hash_table.find(key) != m_hash_table.end()) {
        ...
        if (originator->type() != TYPE_GAME) {
          log_warn(m_log, "Received an interage Directed forward from non-game server %08x,%08x\n",
              msg->get_id1(), msg->get_id2());
        } else {
          ...
          log_msgs(m_log, "TRACK_INTERAGE_FWD from %08x,%08x; forwarded interage Directed message to %u servers\n",
              msg->get_id1(), msg->get_id2(), game_servers.size());
          if (game_servers.size() > 0) {
            game_servers.sort();
            game_servers.unique();
            std::list<ConnectionEntity*>::iterator game;
            for (game = game_servers.begin(); game != game_servers.end(); game++) {
              TrackMsgForward_BackendMessage *next_msg =
                  new TrackMsgForward_BackendMessage((*game)->ipaddr(), (*game)->server_id(), msg, msg->recips_offset());
              (*game)->conn()->enqueue(next_msg);
            }
          }
        }
      } else {
        log_warn(m_log, "Received an interage Directed forward from unknown server %08x,%08x\n",
            msg->get_id1(), msg->get_id2());
        // drop message
      }
    } else {
      if (m_log && m_log->would_log_at(Logger::LOG_WARN)) {
        ...
        m_log->dump_contents(Logger::LOG_WARN, msg->buffer(), msg->message_len());
      }
    }
  }
    break;
  ...
  case TRACK_NEXT_GAMEID: {
    ...
    msg = new TrackNextGameID_BackendMessage(msg->get_id1(), msg->get_id2(), true, how_many, m_next_gameid);
...
Server::reason_t BackendServer::handle_marker(Connection *c, BackendMessage *in) {
  switch (in->type()) {
  case MARKER_NEWGAME: {
    ...
    if (msg->exists()) {
#ifdef USE_PQXX
      try {
        my->C->perform(MarkerGameFind_Request(msg->template_uuid(), game_name, internal_id, game_type, mget_result));
      } catch (const pqxx::in_doubt_error &e) {
      ...
      if (m_log && m_log->would_log_at(Logger::LOG_MSGS)) {
        ...
        if (mget_result == NO_ERROR || mget_result == ERROR_NODE_NOT_FOUND) {
          log_msgs(m_log, "MARKER_NEWGAME opening game %u uuid %s for %u%s\n", internal_id, uuid, msg->player(),
              mget_result == NO_ERROR ? "" : " MISSING");
        } else {
          log_msgs(m_log, "MARKER_NEWGAME opening game %u uuid %s for %u: "
              "error %u\n", internal_id, uuid, msg->player(), mget_result);
        }
      }
      if (mget_result == NO_ERROR || mget_result == ERROR_NODE_NOT_FOUND) {
        MarkerGetGame_BackendMessage *reply = new MarkerGetGame_BackendMessage(in->get_id1(), in->get_id2(), true,
            msg->requester(), (mget_result == NO_ERROR), internal_id, game_type, msg->template_uuid(), &game_name);
        c->enqueue(reply);
      }
      if (mget_result == NO_ERROR) {
...
#ifdef USE_PQXX
        try {
          my->C->perform(MarkerGameMarkers_Request(internal_id, mget_result, allmarkers));
        } catch (const pqxx::in_doubt_error &e) {
        ...
        MarkersAll_BackendMessage *mreply = new MarkersAll_BackendMessage(in->get_id1(), in->get_id2(), internal_id,
            msg->requester(), allmarkers.size());
        if (mget_result == NO_ERROR && allmarkers.size() > 0) {
          std::vector<MarkerGame_MarkerInfo>::iterator a_iter;
          for (a_iter = allmarkers.begin(); a_iter != allmarkers.end(); a_iter++) {
            if (a_iter->marker_id >= 0) {
              ...
              y = htoledouble(y);
              z = htoledouble(z);
              mreply->add_marker(a_iter->marker_id, x, y, z, a_iter->marker_name, a_iter->age_name);
            }
          }
        }
        mreply->finalize();
...
#ifdef USE_PQXX
          try {
            my->C->perform(MarkerGameCaptured_Request(internal_id, msg->player(), mget_result, captured));
          } catch (const pqxx::in_doubt_error &e) {
...
#endif
          log_msgs(m_log, "Sending MARKER_STATE for game %u: %u markers%s\n", internal_id, captured.size(),
              mget_result == NO_ERROR ? "" : " ERROR");
          MarkersCaptured_BackendMessage *creply = new MarkersCaptured_BackendMessage(in->get_id1(), in->get_id2(), internal_id,
              msg->requester(), captured.size());
          if (mget_result == NO_ERROR && captured.size() > 0) {
            std::vector<MarkerGame_CapturedMarker>::iterator c_iter;
            for (c_iter = captured.begin(); c_iter != captured.end(); c_iter++) {
              if (c_iter->marker_id >= 0) {
                creply->add_marker(c_iter->marker_id, c_iter->capture_value);
              }
            }
          }
          creply->finalize();
      ...
      try {
        my->C->perform(
            MarkerGameCreate_Request(msg->player(), msg->name(), internal_id, msg->game_type(), game_uuid, mget_result));
      } catch (const pqxx::in_doubt_error &e) {
      ...
      if (mget_result == NO_ERROR) {
        MarkerGetGame_BackendMessage *reply = new MarkerGetGame_BackendMessage(in->get_id1(), in->get_id2(), true,
            msg->requester(), true, internal_id, msg->game_type(), game_uuid, msg->name());
        c->enqueue(reply);
      }
    }
  }
  ...
  case MARKER_ADD: {
    ...
    try {
      my->C->perform(
          MarkerGameAddMarker_Request(msg->localid(), x, y, z, msg->name(), msg->agename(), marker_num, madd_result));
    } catch (const pqxx::in_doubt_error &e) {
    ...
    if (madd_result == ERROR_NODE_NOT_FOUND) {
      log_warn(m_log, "MARKER_ADD attempted to add marker to non-existent "
          "game %u (requesting GameMgr %08x,%08x %u)\n", msg->localid(), in->get_id1(), in->get_id2(), msg->requester());
    } else {
  ...
  case MARKER_GAME_RENAME: {
    MarkerGameRename_BackendMessage *msg = (MarkerGameRename_BackendMessage*) in;

    status_code_t rename_result = ERROR_INTERNAL;
#ifdef USE_PQXX
    try {
      try {
        my->C->perform(MarkerGameRename_Request(msg->localid(), msg->name(), rename_result));
      } catch (const pqxx::in_doubt_error &e) {
    ...
    log_msgs(m_log, "MARKER_GAME_RENAME %sgame %u to %s%s\n", rename_result == ERROR_NODE_NOT_FOUND ? "non-existent " : "",
        msg->localid(), msg->name()->c_str(), rename_result == ERROR_INTERNAL ? " FAILED" : "");
  ...
  case MARKER_GAME_DELETE: {
    ...
    try {
      try {
        my->C->perform(MarkerGameDelete_Request(msg->localid(), delete_result));
      } catch (const pqxx::in_doubt_error &e) {
  ...
  case MARKER_RENAME: {
    ...
    try {
      try {
        my->C->perform(MarkerGameRenameMarker_Request(msg->localid(), msg->number(), msg->name(), mrename_result));
      } catch (const pqxx::in_doubt_error &e) {
  ...
  case MARKER_DELETE: {
    ...
    try {
      try {
        my->C->perform(MarkerGameDeleteMarker_Request(msg->localid(), msg->number(), mdelete_result));
      } catch (const pqxx::in_doubt_error &e) {
  ...
  case MARKER_CAPTURE: {
    ...
    try {
      try {
        my->C->perform(MarkerGameCaptureMarker_Request(msg->localid(), msg->player(), msg->number(), msg->value(), cap_result));
      } catch (const pqxx::in_doubt_error &e) {
  ...
  case MARKER_GAME_STOP: {
    ...
    try {
      try {
        my->C->perform(MarkerGameStop_Request(msg->localid(), msg->player(), stop_result));
      } catch (const pqxx::in_doubt_error &e) {
...
int32_t BackendServer::init() {
  ...
  try {
    my = new BackendObj(m_log, m_db_addr, m_db_port, m_db_params, m_db_user, m_db_passwd, m_db_name);
...
Server::reason_t BackendServer::message_read(Server::Connection *c, NetworkMessage *msg) {
  ...
  char *m = in->backend_msgtype_c_str_alloc(msg->type());
...
Server::reason_t BackendServer::conn_timeout(Server::Connection *c, Server::reason_t why) {
  if (c == m_timers) {
    struct timeval now;
...
Server::reason_t BackendServer::conn_shutdown(Server::Connection *c, Server::reason_t why) {
  if (c == m_timers) {
  ...
  for (std::vector<DispatcherInfo*>::iterator d_iter = m_dispatchers.begin(); d_iter != m_dispatchers.end(); d_iter++) {
  ...
  // XXX not efficient!
  for (std::map<HashKey, ConnectionEntity*>::iterator iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
      ...
      if (leaver->type() == TYPE_GAME) {
        ...
        for (w_iter = m_timers->begin(); w_iter != m_timers->end(); w_iter++) {
          ...
          if (!memcmp(w->m_ageuuid, leaver->uuid(), UUID_RAW_LEN)) {
            KillClient_BackendMessage::kill_reason_t why = handle_age_request(leaver->uuid(), NULL, true, w->m_id1, w->m_id2,
                w->m_reqid);
            if (why == KillClient_BackendMessage::UNKNOWN) {
              // only need one
              break;
...
#ifdef USE_PQXX
        try {
          my->C->perform(VaultGetAgeByUUID(leaver->uuid(), age_node, age_info, age_fname, db_result));
          if (db_result == NO_ERROR) {
          ...
          if (leaver->server_id() != 0 || leaver->ipaddr() != 0) {
            ...
            if (m_hash_table.find(key) != m_hash_table.end()) {
              ...
              if (gameserver->type() == TYPE_GAME) {
                KillClient_BackendMessage *killit = new KillClient_BackendMessage(leaver->ipaddr(), leaver->server_id(),
                    KillClient_BackendMessage::AUTH_DISCONNECT, leaver->kinum());
...
void BackendServer::Waiter::callback() {
  ...
  if (m_server->m_hash_table.find(key) != m_server->m_hash_table.end()) {
    ...
    TrackAgeRequest_FromBackendMessage *none = new TrackAgeRequest_FromBackendMessage(m_id1, m_id2, m_reqid, ERROR_NO_RESPONSE);
    entity->conn()->enqueue(none);
  } else {
    // client gone?
...
KillClient_BackendMessage::kill_reason_t BackendServer::handle_age_request(const uint8_t *age_uuid, UruString *filename,
  ...
  KillClient_BackendMessage::kill_reason_t result = KillClient_BackendMessage::UNKNOWN;
...
#ifdef USE_PQXX
  try {
    my->C->perform(VaultGetAgeByUUID(age_uuid, age_node, age_info_node, age_fname, db_result));
  } catch (const pqxx::broken_connection &e) {
    // pretty much fatal -- need to shut down or something
    log_err(m_log, "Connection to DB failed!\n");
    db_result = ERROR_DB_TIMEOUT;
...
#ifdef USE_PQXX
      try {
        my->C->perform(VaultCreateAge_Request(fname, fname, fname, fname, age_uuid, NULL, age_node, age_info_node, db_result));
      } catch (const pqxx::in_doubt_error &e) {
  ...
  if (db_result != NO_ERROR) {
    TrackAgeRequest_FromBackendMessage *noage = new TrackAgeRequest_FromBackendMessage(user_id1, user_id2, reqid, db_result);
  ...
  if (filename && (age_fname != *filename)) {
    ...
    TrackAgeRequest_FromBackendMessage *badage = new TrackAgeRequest_FromBackendMessage(user_id1, user_id2, reqid,
        ERROR_INVALID_DATA);
  ...
  for (iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
    if (!memcmp(iter->second->uuid(), age_uuid, UUID_RAW_LEN)) {
      ...
      if (entity->type() != TYPE_GAME) {
        char uuid[UUID_STR_LEN];
        format_uuid(age_uuid, uuid);
        log_warn(m_log, "Found an auth server with the requested game UUID %s!\n", uuid);
        continue;
      }
      if (entity->in_shutdown()) {
        ...
        TrackAddPlayer_FromBackendMessage *bump = new TrackAddPlayer_FromBackendMessage(iter->first.id1(), iter->first.id2(),
      ...
      if (force_new) {
        ...
        Waiter *w = new Waiter(timeout, this, user_id1, user_id2, reqid, user->kinum(), user->name(), user->uuid(), age_uuid,
            age_node);
        m_timers->insert(w);
      }
  ...
  if (iter == m_hash_table.end()) {
    ...
    if (m_dispatchers.size() > 0) {
      ...
      do {
        if (m_dispatchers[m_next_dispatcher]->m_accepting_new_game_servers) {
          ...
          log_debug(m_log, "Telling dispatcher %08x,%08x to start a new %s server\n", disp->m_id1, disp->m_id2,
              age_fname.c_str());
          TrackStartAge_FromBackendMessage *start = new TrackStartAge_FromBackendMessage(disp->m_id1, disp->m_id2,
              new UruString(age_fname), age_uuid);
          disp->m_conn->enqueue(start);
          // and set a timeout
          if (!force_new) {
            Waiter *w = new Waiter(timeout, this, user_id1, user_id2, reqid, user->kinum(), user->name(), user->uuid(),
                age_uuid, age_node);
            m_timers->insert(w);
          }
          got_one = true;
    ...
    if (!got_one) {
      ...
      TrackAgeRequest_FromBackendMessage *none = new TrackAgeRequest_FromBackendMessage(user_id1, user_id2, reqid,
          ERROR_AGE_NOT_FOUND);
...
status_code_t BackendServer::set_player_offline(kinum_t ki, const char *why) {
  ...
  try {
    try {
      my->C->perform(SetPlayerOffline(ki, was_online, player_node, offline));
    } catch (const pqxx::in_doubt_error &e) {
...
void BackendServer::propagate_to_interested(BackendServer::prop_type_t t, uint32_t nodeid, uint32_t child, uint32_t ownerid,
  ...
  if (m_log && m_log->would_log_at(Logger::LOG_DEBUG)) {
    ...
    log_debug(m_log, "propagate_to_interested %s node=%u child=%u\n",
        (t == CHANGED ? "CHANGED" : (t == ADDED ? "ADDED" : "REMOVED")), nodeid, child);
  ...
  if (list_size > 0 || game || tell_all) {
    ...
    case ADDED:
      changed_len = 14;
      changed_buf = new uint8_t[changed_len];
      write16(changed_buf, 0, Auth2Cli_VaultNodeAdded);
      write32(changed_buf, 2, nodeid);
    ...
    case REMOVED:
      changed_len = 10;
      changed_buf = new uint8_t[changed_len];
      write16(changed_buf, 0, Auth2Cli_VaultNodeRemoved);
      write32(changed_buf, 2, nodeid);
      write32(changed_buf, 6, child);
      break;
    default:
    ...
    for (std::map<HashKey, ConnectionEntity*>::iterator iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
      ...
      if (entity->type() == TYPE_AUTH) {
        if (tell_all) {
          reply = new VaultPassthrough_BackendMessage(key.id1(), key.id2(), changed_buf, changed_len, false, false);
          entity->conn()->enqueue(reply);
        } else if (game && game->ipaddr() == entity->ipaddr() && game->server_id() == entity->server_id()) {
...
void BackendServer::propagate_player_delete_to_interested(std::multimap<kinum_t, uint32_t> &notify, uint32_t child) {
  uint8_t msgbuf[10];
  write16(msgbuf, 0, Auth2Cli_VaultNodeRemoved);
  ...
  for (std::map<HashKey, ConnectionEntity*>::iterator iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
    ...
    if (entity->type() == TYPE_AUTH) {
      ...
      for (mapiter = range.first; mapiter != range.second; mapiter++) {
        ...
        VaultPassthrough_BackendMessage *reply = new VaultPassthrough_BackendMessage(key.id1(), key.id2(), msgbuf, 10, false,
            false);
        entity->conn()->enqueue(reply);
      }
    }
...
BackendServer::find_by_uuid(const uint8_t *uuid, uint32_t type) {
  ...
  for (iter = m_hash_table.begin(); iter != m_hash_table.end(); iter++) {
    ConnectionEntity *candidate = iter->second;
    if (candidate->type() == type && !memcmp(candidate->uuid(), uuid, UUID_RAW_LEN)) {
      return candidate;
    }
  }
  return NULL;
